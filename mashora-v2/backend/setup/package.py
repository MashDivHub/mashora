...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'mashora', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/mashora && \
    groupadd -g %(group_id)s mashora && \
    useradd -u %(user_id)s -g mashora mashora -d /var/lib/mashora && \
    mkdir /data && \
    chown mashora:mashora /var/lib/mashora /data
USER mashora
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class MashoraTestTimeoutError(Exception):
    pass


class MashoraTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise MashoraTestError("Installation of package failed")
        else:
        ...
        raise MashoraTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/mashora_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.mashora_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'mashora/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'mashora-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER mashora', DOCKERUSER)
    ...
    def run(self, cmd, build_dir, container_name, user='mashora', exposed_port=None, detach=False, timeout=None):
    ...
    def test_mashora(self):
        logging.info('Starting to test Mashora install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)  # give some time for mashora to install and start
            if os.path.exists(os.path.join(args.build_dir, 'mashora.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise MashoraTestTimeoutError('Mashora pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise MashoraTestError('Error while installing/starting Mashora after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'mashora-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/mashora-*.tar.gz' % self.args.build_dir)[0], '%s/mashora_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/mashora-*.zip' % self.args.build_dir)[0], '%s/mashora_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s mashora"',
            'su mashora -s /bin/bash -c "python3 -m venv /var/lib/mashora/mashoravenv"',
            'su mashora -s /bin/bash -c "/var/lib/mashora/mashoravenv/bin/python3 -m pip install --upgrade pip"',
            'su mashora -s /bin/bash -c "/var/lib/mashora/mashoravenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su mashora -s /bin/bash -c "/var/lib/mashora/mashoravenv/bin/python3 -m pip install /data/src/mashora_{VERSION}.{TSTAMP}.tar.gz"',
            'su mashora -s /bin/bash -c "createdb mycompany"',
            'su mashora -s /bin/bash -c "/var/lib/mashora/mashoravenv/bin/mashora -d mycompany -i base --stop-after-init"',
            'su mashora -s /bin/bash -c "/var/lib/mashora/mashoravenv/bin/mashora -d mycompany --pidfile=/data/src/mashora.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'mashora-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_mashora()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/mashora (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../mashora_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'mashora-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/mashora_{VERSION}.{TSTAMP}_all.deb',
            'su mashora -s /bin/bash -c "mashora -d mycompany -i base --pidfile=/data/src/mashora.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'mashora-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_mashora()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/mashora/rpmbuild'
        build_date = datetime.now().strftime('%a %b %d %Y')
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/mashora.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./mashora-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/mashora-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" --define "%release {TSTAMP}" --define "%build_date {build_date}" /data/src/setup/rpm/mashora.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/mashora*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'mashora-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/mashora-*.noarch.rpm' % self.args.build_dir)[0], '%s/mashora_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s mashora"',
            'su mashora -c "createdb mycompany"',
            'dnf install -q /data/src/mashora_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su mashora -s /bin/bash -c "mashora -c /etc/mashora/mashora.conf -d mycompany -i base --stop-after-init"',
            'su mashora -s /bin/bash -c "mashora -c /etc/mashora/mashora.conf -d mycompany --pidfile=/data/src/mashora.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'mashora-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_mashora()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'mashora-rpm-createrepo-%s' % TSTAMP)
...
class DockerWine(Docker):
    ...
    def __init__(self, args):
        ...
        self.nsi_filepath = r"c:\mashorabuild\server\setup\win32\setup.nsi"
    ...
    def build(self):
        ...
        container_python = '/var/lib/mashora/.wine/drive_c/mashorabuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        self.run(' && '.join(cmds), self.args.build_dir, 'mashora-win-build-%s' % TSTAMP)
...
class DockerIot(DockerWine):
    ...
    def __init__(self, args):
        ...
        self.nsi_filepath = r"c:\mashorabuild\server\setup\win32\setup-iot.nsi"
        self.nt_service_name = "mashora-iot"

    def build_image(self):
        shutil.copy(os.path.join(self.args.build_dir, 'mashora/addons/iot_box_image/configuration/requirements.txt'), self.docker_dir / 'requirements-iot.txt')
...
def parse_args():
    ...
    parsed_args.mashora_dir = ROOTDIR
    return parsed_args
