...
!define PRODUCT_NAME "Mashora IoT"
...
OutFile "${TOOLSDIR}\server\mashora_iot_setup_${VERSION}.exe"
...
; English
LangString DESC_Mashora_IoT ${LANG_ENGLISH} "Install the Mashora Server with IoT modules."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Mashora for Partnership and/or Support"
LangString TITLE_Mashora_IoT ${LANG_ENGLISH} "Mashora IoT"
...
; French
LangString DESC_Mashora_IoT ${LANG_FRENCH} "Installation du Serveur Mashora avec les modules IoT."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Mashora pour un Partenariat et/ou du Support"
LangString TITLE_Mashora_IoT ${LANG_FRENCH} "Mashora IoT"
...
Section $(TITLE_Mashora_IoT) SectionMashora_IoT
    ...
    # Cloning mashora
    DetailPrint "Cloning Mashora"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" clone --filter=tree:0 -b 19.0 --single-branch --no-checkout https://github.com/mashora/mashora.git "$INSTDIR\mashora"'

    DetailPrint "Configuring Sparse Checkout for IoT modules"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\mashora" sparse-checkout init --no-cone'
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\mashora" sparse-checkout set addons/web addons/hw_* addons/iot_drivers addons/iot_base addons/iot_box_image/configuration addons/point_of_sale/tools/posbox/configuration/requirements.txt mashora mashora-bin'

    DetailPrint "Checking out the repository"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\mashora" checkout'
    ...
    DetailPrint "Configuring $(TITLE_Mashora_IoT)"
    # Set data_dir
    WriteIniStr "$INSTDIR\mashora.conf" "options" "data_dir" "$INSTDIR\sessions"
    # set modules to load
    WriteIniStr "$INSTDIR\mashora.conf" "options" "server_wide_modules" "iot_drivers,web"
    # Configure logging
    WriteIniStr "$INSTDIR\mashora.conf" "options" "logfile" "$INSTDIR\mashora.log"
    WriteIniStr "$INSTDIR\mashora.conf" "options" "log_handler" ":WARNING"
    WriteIniStr "$INSTDIR\mashora.conf" "options" "log_level" "warn"
    # Other configuration
    WriteIniStr "$INSTDIR\mashora.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\mashora.conf" "options" "max_cron_threads" "0"

    DetailPrint "Installing Windows service"
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\mashora\mashora-bin\" -c "\"$INSTDIR\mashora.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\mashora\mashora-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
    WriteIniStr "$INSTDIR\mashora.conf" "options" "proxy_access_token" "$ProxyTokenPwd"
...
Section -$(TITLE_Nginx) Nginx
    ...
    File "..\..\mashora\addons\iot_box_image\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\mashora\addons\iot_box_image\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionMashora_IoT} $(DESC_Mashora_IoT)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Mashora IoT ${VERSION}"
