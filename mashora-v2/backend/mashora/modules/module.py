...
import mashora.tools as tools
import mashora.upgrade
...
class UpgradeHook:
    """Makes the legacy `migrations` package being `mashora.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^mashora\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("mashora.addons.base.maintenance.migrations", "mashora.upgrade")
...
def initialize_sys_path() -> None:
    """
    Setup the addons path ``mashora.addons.__path__`` with various defaults
    ...
    for path in (
        ...
        *tools.config['addons_path'],
        ...
        if os.access(path, os.R_OK) and path not in mashora.addons.__path__:
            mashora.addons.__path__.append(path)

    # hook mashora.upgrade on upgrade-path
    ...
    for up in tools.config['upgrade_path'] or [legacy_upgrade_path]:
        if up not in mashora.upgrade.__path__:
            mashora.upgrade.__path__.append(up)

    # create decrecated module alias from mashora.addons.base.maintenance.migrations to mashora.upgrade
    spec = importlib.machinery.ModuleSpec("mashora.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = mashora.upgrade  # type: ignore
    sys.modules["mashora.addons.base.maintenance"] = maintenance_pkg
    sys.modules["mashora.addons.base.maintenance.migrations"] = mashora.upgrade

    # hook for upgrades and namespace freeze
    if not getattr(initialize_sys_path, 'called', False):  # only initialize once
        mashora.addons.__path__._path_finder = lambda *a: None  # prevent path invalidation
        mashora.upgrade.__path__._path_finder = lambda *a: None  # prevent path invalidation
...
class Manifest(Mapping[str, typing.Any]):
    ...
    def _get_manifest_from_addons(module: str) -> Manifest | None:
        ...
        for adp in mashora.addons.__path__:
...
def get_resource_from_path(path: str) -> tuple[str, str, str] | None:
    ...
    """
    resource = None
    sorted_paths = sorted(mashora.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name: str) -> None:
    ...
    qualname = f'mashora.addons.{module_name}'
