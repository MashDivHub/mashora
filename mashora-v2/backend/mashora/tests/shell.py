...
TEST_MODULE_NAME_PATTERN = re.compile(r'^mashora\.addons\.\w+\.tests')


def run_tests(env, test_tags, modules=None, reload_tests=False):
    """Run tests for the given modules and test tags."""

    if mashora.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in mashora shell')
        return

    if mashora.tools.config['workers'] != 0:
        _logger.error('run_tests should be used only in threaded mode')
        return

    from mashora.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    mashora.tools.config['test_tags'] = test_tags
    mashora.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    mashora.tools.config['test_enable'] = None
    mashora.tools.config['test_tags'] = None
