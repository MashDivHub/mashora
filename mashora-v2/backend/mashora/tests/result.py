...
MASHORA_TEST_MAX_FAILED_TESTS = max(1, int(os.environ.get('MASHORA_TEST_MAX_FAILED_TESTS', sys.maxsize)))

stats_logger = logging.getLogger('mashora.tests.stats')
...
class MashoraTestResult(object):
    """
    ...
    def _checkShouldStop(self):
        if self.total_errors_count() >= MASHORA_TEST_MAX_FAILED_TESTS:
            ...
            if not global_report.shouldStop:
                _logger.error(
                    ...
                    "Remaining tests will be skipped.", MASHORA_TEST_MAX_FAILED_TESTS)
