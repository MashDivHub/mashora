# -*- coding: utf-8 -*-
"""
The module :mod:`mashora.tests.common` provides unittest test cases and a few
...
import mashora.addons.base
import mashora.cli
import mashora.http
import mashora.models
import mashora.orm.registry
from mashora import api
from mashora.exceptions import AccessError
from mashora.fields import Command
from mashora.modules.registry import Registry, DummyRLock
from mashora.service import security
from mashora.sql_db import Cursor, Savepoint
from mashora.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from mashora.tools.mail import single_email_re
from mashora.tools.misc import find_in_path, lower_logging
from mashora.tools.xml_utils import _validate_xml
from mashora.addons.base.models import ir_actions_report

from . import case, test_cursor
from .result import MashoraTestResult
...
if mashora.cli.COMMAND in ('server', 'start') and not config['test_enable']:
    _logger.error(
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: mashora.tests.common.Form is deprecated, use mashora.tests.Form",
...
# The mashora library is supposed already configured.
HOST = '127.0.0.1'
...
def release_test_lock():
    ...
    try:
        ...
        if not _registry_test_lock.acquire(timeout=60):
            tag = mashora.modules.module.current_test.canonical_tag
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Mashora module name.
    """
    def register(func):
        # register func by mashora module name
        if func.__module__.startswith('mashora.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most mashora operations;
    """
    if not login:
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for Mashora-specific code. This class is abstract and
    ...
    def __init_subclass__(cls):
        ...
        if cls.__module__.startswith('mashora.addons.'):
            if getattr(cls, 'test_tags', None) is None:
                cls.test_tags = {'standard', 'at_install'}
            cls.test_module = cls.__module__.split('.')[2]

    longMessage = True      # more verbose error message by default: https://www.mashora.com/r/Vmh
    warm = True             # False during warm-up phase (see :func:`warmup`)
    _python_version = sys.version_info

    _tests_run_count = int(os.environ.get('MASHORA_TEST_FAILURE_RETRIES', 0)) + 1
    ...
    def run(self, result: MashoraTestResult) -> None:
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~mashora.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(mashora.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            mashora.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = mashora.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('mashora.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/mashora/addons/" in filename:
                        filename = filename.rsplit("/mashora/addons/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(
            self,
            records: mashora.models.BaseModel,
        ...
        for record in records:
            ...
            for field_name in field_names:
                ...
                match record._fields[field_name]:
                    case mashora.fields.Many2one():
                        record_value = record_value.id
                    case mashora.fields.One2many() | mashora.fields.Many2many():
                        record_value = sorted(record_value.ids)
                    case mashora.fields.Float() as field if digits := field.get_digits(record.env):
                        record_value = Approx(record_value, digits[1], decorate=False)
                    case mashora.fields.Monetary() as field if currency_field_name := field.get_currency_field(record):
    ...
    def assertCanOpenTestCursor(self):
        ...
        if mashora.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {mashora.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = mashora.http.request
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if mashora.tools.config['test_tags'] and 'is_query_count' in mashora.tools.config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | mashora.addons.base.models.res_currency.ResCurrency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    """
    muted_registry_logger = mute_logger(mashora.orm.registry._logger.name)
    freeze_time = None
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('mashora.addons.base.models.res_users.ResUsersPatchedInTest._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f", loglevel=logging.RUNBOT, directory=''):
    ...
    dest = pathlib.Path(mashora.tools.config['screenshots']) / get_db_name() / directory
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        if scs := mashora.tools.config['screencasts']:
    ...
    def _chrome_start(
        ...
        switches = {
            ...
            '--enable-logging': '',
            '--v': str(int(os.environ.get("MASHORA_BROWSER_LOG_VERBOSITY", "0"))),
            '--no-first-run': '',
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Mashora widgets are created they all appear as Class(...)
...
def _find_executable():
    browser_bin_path = os.environ.get('MASHORA_BROWSER_BIN')  # used for testing specific Chrome builds
...
class HttpCase(TransactionCase):
    ...
    touch_enabled = False
    session: mashora.http.Session = None
    ...
    @classmethod
    def http_port(cls):
        if mashora.service.server.server is None:
            return None
        return mashora.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('mashora.service.http.request.')]
        ...
        if request_threads:
            ...
            mashora.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        mashora.http.root.session_store.save(self.session)

    def authenticate(self, user, password, *,
        browser: ChromeBrowser = None, session_extra: dict | None = None):
        if getattr(self, 'session', None):
            mashora.http.root.session_store.delete(self.session)

        self.session = session = mashora.http.root.session_store.new()
        session.update(
            mashora.http.get_default_session(),
            db=get_db_name(),
            # In order to avoid perform a query to each first `url_open`
            # in a test (insert `res.device.log`).
            _trace_disable=True,
        )
        session.context['lang'] = mashora.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('mashora.addons.base.models.res_users.ResUsersPatchedInTest._check_credentials', new=patched_check_credentials):
        ...
        mashora.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        with self.allow_requests(browser=browser), contextlib.ExitStack() as atexit:
            ...
            if "bus.bus" in self.env.registry:
                from mashora.addons.bus.websocket import CloseCode, _kick_all, WebsocketConnectionHandler  # noqa: PLC0415
                from mashora.addons.bus.models.bus import BusBus  # noqa: PLC0415
            ...
            cpu_throttling_os = os.environ.get('MASHORA_BROWSER_CPU_THROTTLING')  # used by dedicated runbot builds
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("MASHORA_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"mashora.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"mashora.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('mashora.http.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if mashora.tools.config['test_tags'] and 'is_tour' in mashora.tools.config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from mashora.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in Mashora test suites
