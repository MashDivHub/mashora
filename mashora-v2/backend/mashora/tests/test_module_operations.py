...
from mashora.tools import config, topological_sort, unique, profiler
from mashora.modules.registry import Registry
from mashora.netsvc import init_logger
from mashora.tests import standalone_tests
import mashora.tests.loader

_logger = logging.getLogger('mashora.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Mashora modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=addons_path,
        help="Comma-separated list of paths to directories containing extra Mashora modules")
...
def test_standalone(args):
    ...
    mashora.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        mashora.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    if args.addons_path:
        ...
        mashora.modules.module.initialize_sys_path()
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'mashora.modules.loading': {'level': 'CRITICAL'},
            'mashora.sql_db': {'level': 'CRITICAL'},
            'mashora.models.unlink': {'level': 'WARNING'},
            'mashora.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
