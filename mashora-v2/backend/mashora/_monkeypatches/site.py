...
mashora.evented = False


def patch_evented():
    """Running mode flags (gevent, prefork)

    This should be executed early. It will initialize the `mashora.evented` variable.
    """
    if mashora.evented or not (len(sys.argv) > 1 and sys.argv[1] == 'gevent'):
        return
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in mashora/debian/copyright)
        while 1:
            state = conn.poll()
