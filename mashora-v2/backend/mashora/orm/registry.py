...
from mashora.tools.func import locked, reset_cached_properties
from mashora.tools.lru import LRU
from mashora.tools.misc import Collector, format_frame
...
if typing.TYPE_CHECKING:
    ...
    from mashora.fields import Field
    from mashora.models import BaseModel
    from mashora.sql_db import BaseCursor, Connection, Cursor
    from mashora.modules import module_graph


_logger = logging.getLogger('mashora.registry')
_schema = logging.getLogger('mashora.schema')
...
class Registry(Mapping[str, type["BaseModel"]]):
        ...
        *,
        ...
        try:
            ...
            with registry.cursor() as cr:
                ...
                from mashora.modules import db  # noqa: PLC0415
                if db.is_initialized(cr):
                    cr.execute("DELETE FROM ir_config_parameter WHERE key='base.partially_updated_database'")
                    if cr.rowcount:
                        update_module = True
            # This should be a method on Registry
            from mashora.modules.loading import load_modules, reset_modules_state  # noqa: PLC0415
    ...
    def init(self, db_name: str) -> None:
        ...
        if config['test_enable']:
            from mashora.tests.result import MashoraTestResult  # noqa: PLC0415
            self._assertion_report: MashoraTestResult | None = MashoraTestResult()
        else:
        ...
        for indexname, tablename, field in expected:
            ...
            if index and indexname not in existing:
                ...
                if index == 'trigram':
                    ...
                    from mashora.modules.db import FunctionStatus  # noqa: PLC0415
