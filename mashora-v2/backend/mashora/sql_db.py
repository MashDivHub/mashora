...
if typing.TYPE_CHECKING:
    ...
    from mashora.orm.environments import Transaction
...
class Cursor(BaseCursor):
    ...
    def __init__(self, pool: ConnectionPool, dbname: str, dsn: dict):
        ...
        if os.getenv('MASHORA_FAKETIME_TEST_MODE') and self.dbname in tools.config['db_name']:
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # Mashora Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri: str, readonly=False) -> tuple[str, dict]:
    ...
    if 'MASHORA_PGAPPNAME' in os.environ:
        warnings.warn("Since 19.0, use PGAPPNAME instead of MASHORA_PGAPPNAME", DeprecationWarning)
        app_name = os.environ['MASHORA_PGAPPNAME']
...
def db_connect(to: str, allow_uri=False, readonly=False) -> Connection:
    ...
    maxconn = (tools.config['db_maxconn_gevent'] if hasattr(mashora, 'evented') and mashora.evented else 0) or tools.config['db_maxconn']
...
def close_db(db_name: str) -> None:
    """ You might want to call mashora.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:
