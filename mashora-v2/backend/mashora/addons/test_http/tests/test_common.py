...
import mashora
from mashora.http import Session
from mashora.addons.base.tests.common import HttpCaseWithUserDemo
from mashora.tools import config, reset_cached_properties
from mashora.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(mashora.http.Application, 'session_store', session_store)
        cls.classPatch(mashora.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(mashora.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        mashora.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('mashora.http.db_list') as db_list, \
             patch('mashora.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('mashora.http.db_list') as db_list, \
             patch('mashora.http.db_filter') as db_filter, \
             patch('mashora.http.Registry') as Registry:
