...
import mashora
from mashora.http import content_disposition, root
from mashora.tests import tagged
from mashora.tests.common import HOST, BaseCase, get_db_name, new_test_user
from mashora.tools import config, file_path, mute_logger, parse_version

from .test_common import TestHttpBase
from mashora.addons import test_http
from mashora.addons.test_http.controllers import CT_JSON
from mashora.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc1_reverse_proxy(self):
        # client <-> reverse-proxy <-> mashora
        client_ip = '127.0.0.16'
        reverseproxy_ip = gethostbyname(HOST)
        host = 'mycompany.mashora.com'
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = mashora.tools.misc.DotDict(db=False)
            return mashora.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'mashora.com/{uri}', host='mashora.com'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://mashora.com/{uri}', host='mashora.com'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'mashora.com/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://mashora.com/{uri}'), "No host allowed")
    ...
    def test_misc4_rpc_qweb(self):
        ...
        for method in (self.db_url_open, self.nodb_url_open):
            with self.subTest(method=method.__name__):
                with mute_logger('mashora.addons.rpc.controllers.jsonrpc'):
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'mashora.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict(mashora.tools.config.options, {'proxy_mode': True}):
    ...
    def test_misc8_concurrency_error(self):
        with (
            self.assertLogs('mashora.service.model') as log_catcher,
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(mashora.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = mashora.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            mashora.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')
