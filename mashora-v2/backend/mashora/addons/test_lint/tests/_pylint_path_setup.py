...
def register(linter):
    ...
    if addons_path:
        for p in addons_path.split(os.pathsep):
            ...
            mashora.addons.__path__.append(normp)

def modpath_from_file(filename, path):
    if filename == "./addons":
        return ['mashora', 'addons']
...
class AddonsPackageFinder(spec.Finder):
    """Finder which special cases mashora and mashora.addons, such that they're
    ...
    def find_module(
        ...
        if modname in ('mashora', 'mashora.addons'):
