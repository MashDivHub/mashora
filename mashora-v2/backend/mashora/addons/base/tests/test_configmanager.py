...
import mashora
from mashora.tests import TransactionCase
from mashora.tools import file_open, file_open_temporary_directory, file_path
from mashora.tools.config import configmanager

EMPTY_CONFIG_PATH = file_path('base/tests/config/empty.conf')
PROJECT_PATH = mashora.tools.config.root_path.removesuffix('/mashora')
DEFAULT_DATADIR = mashora.tools.config._default_options['data_dir']

MISSING_HTTP_INTERFACE = """\
WARNING:mashora.tools.config:missing --http-interface/http_interface, \
...
class TestConfigManager(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        patcher = patch.dict('os.environ', {'MASHORA_RC': EMPTY_CONFIG_PATH}, clear=True)
        patcher.start()
    ...
    def test_01_default_config(self):
        self.assertConfigEqual({
            ...
            'proxy_access_token': '',
            'publisher_warranty_url': 'http://services.mashora.com/publisher-warranty/',
            'reportgz': False,
    ...
    def test_02_config_file(self):
        ...
        with self.assertLogs('mashora.tools.config', 'WARNING') as capture:
        ...
        self.assertEqual(capture.output, [
            "WARNING:mashora.tools.config:option addons_path, no such directory '/tmp/mashora', skipped",
            "WARNING:mashora.tools.config:option upgrade_path, no such directory '/tmp/upgrade', skipped",
            "WARNING:mashora.tools.config:option pre_upgrade_scripts, no such file '/tmp/pre-custom.py', skipped",
        ])
    ...
    def test_04_mashora16_config_file(self):
        # test that loading the Mashora 16.0 generated default config works
        ...
        with (
            ...
            self.assertLogs('mashora.tools.config', 'WARNING') as capture_warn,
        ):
        ...
        def missing(*options):
            return [
                f"WARNING:mashora.tools.config:unknown option '{option}' in "
        ...
        def falsy(*options):
            return [
                f"WARNING:mashora.tools.config:option {option} reads 'False' "
    ...
    def test_06_cli(self):
        ...
        self.assertEqual(capture.output, [
            "WARNING:mashora.tools.config:test file '/tmp/file-file' cannot be found",
        ])
        ...
        self.assertConfigEqual({
            ...
            'log_handler': [
                ':WARNING',
                'mashora.tools.config:DEBUG',
                'mashora.http:DEBUG',
                'mashora.sql_db:DEBUG',
            ],
            'log_db': 'logdb',
            'log_db_level': 'debug',
            'log_level': 'debug',
        ...
        for args, stop_after_init in [
            ...
            with self.subTest(args=args):
                if any('--test' in arg for arg in args):
                    ...
                    self.assertEqual(capture.output, [
                        "WARNING:mashora.tools.config:Empty -d/--database/db_name, tests won't run",
                    ])
                else:
                    _, options = self.parse_reset(args)
