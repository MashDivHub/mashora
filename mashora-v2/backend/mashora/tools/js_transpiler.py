"""
This code is what let us use ES6-style modules in mashora.
Classic Mashora modules are composed of a top-level :samp:`mashora.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@mashora-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** mashora-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual mashora one with minimal changes.
...
from mashora.tools.misc import OrderedSet


def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom mashora modules.
    ...
    legacy_mashora_define = get_aliased_mashora_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_mashora_define, module_path, dependencies),
        partial(convert_t, url)
    ]
    for s in steps:
        content = s(content)
    if legacy_mashora_define:
        content += legacy_mashora_define
    return content
...
def url_to_module_path(url):
    """
    Mashora modules each have a name. (mashora.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_mashora_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the mashora.define call.
    ...
    """
    return f"""mashora.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
MASHORA_MODULE_RE = re.compile(r"""
    \s*                                # starting white space
    \/(\*|\/)                          # /* or //
    .*                                 # any comment in between (optional)
    @mashora-module                       # '@mashora-module' statement
...
def is_mashora_module(url, content):
    """
    Detect if the file is a native mashora module.
    We look for a comment containing @mashora-module.

    :param url:
    :param content: source code
    :return: is this a mashora module that need transpilation ?
    """
    result = MASHORA_MODULE_RE.match(content)
...
def get_aliased_mashora_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the mashora source base being:
    ...
    > mashora.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @mashora-module */
        // after
        /** @mashora-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @mashora-module */
        // after
        /** @mashora-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = MASHORA_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nmashora.define(`%s`, ['%s'], function (require) {
                        return require('%s');
