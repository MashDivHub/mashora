...
patch(Colibri.prototype, {
    protectSyncAfterAsync(interaction, name, fn) {
        ...
        return (...args) => {
            ...
            wysiwyg?.mashoraEditor.observerUnactive(fullName);
            const result = fn(...args);
            wysiwyg?.mashoraEditor.observerActive(fullName);
            return result;
        };
    },
    addListener(target, event, fn, options) {
        ...
        const wysiwyg = window.$?.("#wrapwrap").data("wysiwyg");
        let stealthFn = fn;
        if (wysiwyg?.mashoraEditor && !fn.isHandler && stealth) {
            const name = `${this.interaction.constructor.name}/${event}`;
            stealthFn = (...args) => {
                wysiwyg.mashoraEditor.observerUnactive(name);
                const result = fn(...args);
                wysiwyg.mashoraEditor.observerActive(name);
                return result;
            };
        }
        return super.addListener(target, event, stealthFn, options);
