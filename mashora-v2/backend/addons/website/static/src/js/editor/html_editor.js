    ...
    isFrontendUrl(url) {
        ...
        return (
            (browser.location.hostname === parsedUrl.hostname ||
                // Also check if the mashora-hosted domain is the current domain of the url
                new RegExp(`^https?://${session.db}\\.mashora\\.com(/.*)?$`).test(parsedUrl.origin)) &&
            !parsedUrl.pathname.startsWith("/mashora") &&
