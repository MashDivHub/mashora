...
class TestWebsiteAssets(mashora.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/mashora')
        check_asset()
        self.url_open(domain_2 + '/mashora')
        check_asset()
        self.url_open(domain_1 + '/mashora')
        check_asset()
        self.url_open(domain_2 + '/mashora')
...
class TestWebAssets(mashora.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with mashora.tools.mute_logger('mashora.addons.web.controllers.binary'):
            self.assertEqual(
