...
# Mashora
mashora_url = 'http://localhost:8069'
mashora_db = 'mashora_db'
mashora_username = 'admin'
mashora_password = 'admin'
...
def get_blobs_to_be_deleted(blob_urls, batch_size=1000):
    common = xmlrpc.client.ServerProxy(f'{mashora_url}/xmlrpc/2/common')
    uid = common.authenticate(mashora_db, mashora_username, mashora_password, {})
    models = xmlrpc.client.ServerProxy(f'{mashora_url}/xmlrpc/2/object')
    for blob_urls_ in split_every(batch_size, blob_urls):
        blob_urls_ = list(blob_urls_)
        attachments = models.execute_kw(mashora_db, uid, mashora_password, 'ir.attachment', 'search_read', [
