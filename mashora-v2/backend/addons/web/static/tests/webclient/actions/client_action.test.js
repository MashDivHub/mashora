...
import { animationFrame, runAllTimers } from "@mashora/hoot-mock";
...
test("test reload client action", async () => {
    ...
    await runAllTimers();
    expect.verifySteps([
        "replaceState /mashora?test=42",
        "window_reload",
        "pushState /mashora/action-2",
        "window_reload",
        "pushState /mashora?menu_id=1",
        "window_reload",
        "pushState /mashora/action-1?menu_id=2",
        "window_reload",
    ]);
});
...
test("test display_exception client action", async () => {
    ...
    getService("action").doAction({
        ...
        params: {
            ...
            data: {
                name: `mashora.exceptions.UserError`,
                debug: "traceback",
                arguments: [],
                context: {},
