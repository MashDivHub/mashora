import { after, beforeEach, describe, expect, getFixture, test } from "@mashora/hoot";
 ...
 * @param {(env: import("@web/env").MashoraEnv, dependencies: Record<string, any>) => any} factory
 */
...
test(`mountComponent creates an env and sets the application as root when no env is provided`, async () => {
    ...
    after(() => {
        delete mashora.__WOWL_DEBUG__;
    });
    const { env } = app;
    expect(env.services).toEqual({ my_service: "a" });
    expect(mashora.__WOWL_DEBUG__).toEqual({ root: app.root.component });
...
test(`mountComponent uses the env when provided and doesn't start the services`, async () => {
    ...
    expect(mashora.__WOWL_DEBUG__).toBe(undefined);
    expect(getFixture()).toHaveText("Root");
});
