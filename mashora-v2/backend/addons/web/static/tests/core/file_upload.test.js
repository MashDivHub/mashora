import { expect, test, waitFor } from "@mashora/hoot";
import {
    contains,
    getService,
    mockService,
...
test("handles Mashora's jsonrpc error", async () => {
    await mountWithCleanup(Parent);
    onRpc("/test/", () => ({
        error: {
            data: {
                name: "ValidationError",
                message: "Boom Mashora",
            },
        },
    }));
    const fileUploadService = await getService("file_upload");
    fileUploadService.upload("/test/", []);
    await waitFor(".o_notification:has(.bg-danger):contains(ValidationError: Boom Mashora)");
});
