...
import { htmlEscape, markup } from "@mashora/owl";
...
test("mashoramark", () => {
    expect(mashoramark("").toString()).toBe("");
    expect(mashoramark("**test**").toString()).toBe("<b>test</b>");
    expect(mashoramark("**test** something else **test**").toString()).toBe(
        "<b>test</b> something else <b>test</b>"
    );
    expect(mashoramark("--test--").toString()).toBe(`<span class="text-muted">test</span>`);
    expect(mashoramark("--test-- something else --test--").toString()).toBe(
        `<span class="text-muted">test</span> something else <span class="text-muted">test</span>`
    );
    expect(mashoramark("`test`").toString()).toBe(
    ...
    expect(mashoramark("test\ttest2").toString()).toBe(
        `test<span style="margin-left: 2em"></span>test2`
    );
    expect(mashoramark("test\ntest2").toString()).toBe("test<br>test2");
    expect(mashoramark("<p>**test**</p>").toString()).toBe("&lt;p&gt;<b>test</b>&lt;/p&gt;");
    expect(mashoramark(markup`<p>**test**</p>`).toString()).toBe("<p><b>test</b></p>");
});
