...
test("escape", () => {
    ...
    expect(escape(`<a href="https://www.mashora.com">mashora<a>`)).toBe(
        `&lt;a href=&quot;https://www.mashora.com&quot;&gt;mashora&lt;a&gt;`
    );
    expect(escape(`<a href='https://www.mashora.com'>mashora<a>`)).toBe(
        `&lt;a href=&#x27;https://www.mashora.com&#x27;&gt;mashora&lt;a&gt;`
    );
    expect(escape("<a href='https://www.mashora.com'>Mashora`s website<a>")).toBe(
        `&lt;a href=&#x27;https://www.mashora.com&#x27;&gt;Mashora&#x60;s website&lt;a&gt;`
    );
});
...
test("escapeRegExp", () => {
    ...
    expect(escapeRegExp("[wowl]")).toBe("\\[wowl\\]");
    expect(escapeRegExp("[wowl.mashora]")).toBe("\\[wowl\\.mashora\\]");
    expect(escapeRegExp("^mashora.define([.]*)$")).toBe("\\^mashora\\.define\\(\\[\\.\\]\\*\\)\\$");
    expect(escapeRegExp("[.*+?^${}()|[]\\")).toBe("\\[\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
});
...
test("isEmail", () => {
    ...
    expect(isEmail("test@mashora")).toBe(false);
    expect(isEmail("test@mashora@mashora.com")).toBe(false);
    expect(isEmail("te st@mashora.com")).toBe(false);

    expect(isEmail("test@mashora.com")).toBe(true);
});
