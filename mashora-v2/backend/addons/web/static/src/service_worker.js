...
const cacheName = "mashora-sw-cache";
const homepageURL = "/mashora";
...
const extractSessionInfo = (htmlContent) => {
    const match = htmlContent.match(/mashora\.__session_info__\s*=\s*({.*?});/s);
...
const readDataOnCache = async (url) => {
    ...
    // if you come from /mashora to project the url is now /mashora/project, but it doesn't exist in cache so use /mashora instead
    if (!response) {
...
const serveShareTarget = (event) => {
    ...
    event.respondWith(Response.redirect("/mashora?share_target=trigger"));
    event.waitUntil(
        (async () => {
            // The page sends this message to tell the service worker it's ready to receive the file.
            await waitingMessage("mashora_share_target");
            const client = await self.clients.get(event.resultingClientId || event.clientId);
            const data = await event.request.formData();
            client.postMessage({
                shared_files: data.getAll("externalMedia") || [],
                action: "mashora_share_target_ack",
            });
        })()
    );
};
