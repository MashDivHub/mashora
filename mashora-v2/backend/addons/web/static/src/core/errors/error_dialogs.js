...
import { Component, useRef, useState, markup } from "@mashora/owl";
...
export const mashoraExceptionTitleMap = new Map(
    Object.entries({
        "mashora.addons.base.models.ir_mail_server.MailDeliveryException": _t("MailDeliveryException"),
        "mashora.exceptions.AccessDenied": _t("Access Denied"),
        "mashora.exceptions.MissingError": _t("Missing Record"),
        "mashora.addons.web.controllers.action.MissingActionError": _t("Missing Action"),
        "mashora.addons.base.models.ir_actions.ServerActionWithWarningsError": _t("Invalid Operation"),
        "mashora.exceptions.UserError": _t("Invalid Operation"),
        "mashora.exceptions.ValidationError": _t("Validation Error"),
        "mashora.exceptions.AccessError": _t("Access Error"),
        "mashora.exceptions.Warning": _t("Warning"),
    })
);
...
ClientErrorDialog.title = _t("Mashora Client Error");

// -----------------------------------------------------------------------------
// Network Error Dialog
// -----------------------------------------------------------------------------
export class NetworkErrorDialog extends ErrorDialog {}
NetworkErrorDialog.title = _t("Mashora Network Error");
...
export class RPCErrorDialog extends ErrorDialog {
    ...
    inferTitle() {
        ...
        if (this.props.exceptionName && mashoraExceptionTitleMap.has(this.props.exceptionName)) {
            this.title = mashoraExceptionTitleMap.get(this.props.exceptionName).toString();
            return;
        }
        ...
        switch (this.props.type) {
            case "server":
                this.title = _t("Mashora Server Error");
                break;
            case "script":
                this.title = _t("Mashora Client Error");
                break;
            case "network":
                this.title = _t("Mashora Network Error");
                break;
        }
    }
...
export class WarningDialog extends Component {
    ...
    inferTitle() {
        if (this.props.exceptionName && mashoraExceptionTitleMap.has(this.props.exceptionName)) {
            return mashoraExceptionTitleMap.get(this.props.exceptionName).toString();
        }
        return this.props.title || _t("Mashora Warning");
    }
}
...
export class RedirectWarningDialog extends Component {
    ...
    setup() {
        ...
        this.title = capitalize(subType) || _t("Mashora Warning");
        this.message = message;
...
registry
    ...
    .add("mashora.exceptions.AccessDenied", WarningDialog)
    .add("mashora.exceptions.AccessError", WarningDialog)
    .add("mashora.exceptions.MissingError", WarningDialog)
    .add("mashora.addons.web.controllers.action.MissingActionError", WarningDialog)
    .add("mashora.addons.base.models.ir_actions.ServerActionWithWarningsError", WarningDialog)
    .add("mashora.exceptions.UserError", WarningDialog)
    .add("mashora.exceptions.ValidationError", WarningDialog)
    .add("mashora.exceptions.RedirectWarning", RedirectWarningDialog)
    .add("mashora.http.SessionExpiredException", SessionExpiredDialog)
