...
from mashora.http import content_disposition, dispatch_rpc, request, Response
from mashora.service import db
from mashora.tools.misc import file_open, str2bool
from mashora.tools.translate import _

from mashora.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = mashora.tools.config.verify_admin_password('admin')
        d['list_db'] = mashora.tools.config['list_db']
        d['langs'] = mashora.service.db.exp_list_lang()
        d['countries'] = mashora.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = mashora.service.db.list_db_incompatible(d['databases'])
        except mashora.exceptions.AccessDenied:
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = mashora.tools.config.verify_admin_password('admin')
        ...
        try:
            ...
            with mashora.modules.registry.Registry(name).cursor() as cr:
            ...
            return request.redirect('/mashora')
    ...
    def backup(self, master_pwd, name, backup_format='zip', filestore=True):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            mashora.service.db.check_super(master_pwd)
            ...
            dump_stream = mashora.service.db.dump_db(name, None, backup_format, filestore)
