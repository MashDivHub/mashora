import { beforeEach, expect, test, describe, getFixture } from "@mashora/hoot";
import { setSelection } from "./_helpers/selection";
import { click, hover, queryOne, waitFor, waitForNone } from "@mashora/hoot-dom";
import {
    contains,
    defineModels,
    fields,
...
import { Component, onMounted, onWillUnmount, xml } from "@mashora/owl";
...
test("Toolbar should not overflow scroll container", async () => {
    ...
    const editable = queryOne(".mashora-editor-editable");
...
test("Toolbar should be visible after scroll bar is added", async () => {
    ...
    const p = queryOne(".mashora-editor-editable p");
...
test("Toolbar should not overflow scroll container at the bottom", async () => {
    ...
    const lastP = queryOne(".mashora-editor-editable p:last-child");
...
describe("powerbox", () => {
    ...
    test("Powerbox should be visible in a editable with small height", async () => {
        ...
        setSelection({ anchorNode: queryOne(".mashora-editor-editable p"), anchorOffset: 1 });
        insertText(editor, "/");
    ...
    test("Powerbox should be visible in a editable with small height (2)", async () => {
        ...
        const thirdP = queryOne(".mashora-editor-editable p:nth-child(3)");
...
test("Table column control should always be displayed on top of the table", async () => {
    ...
    const scrollableElement = queryOne(".o_content");
    const table = queryOne(".mashora-editor-editable table");
    await hover(".mashora-editor-editable td");
