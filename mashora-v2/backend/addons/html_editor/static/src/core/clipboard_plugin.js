...
export class ClipboardPlugin extends Plugin {
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePasteMashoraEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteMashoraEditorHtml(clipboardData) {
        const mashoraEditorHtml = clipboardData.getData("application/vnd.mashora.mashora-editor");
        const textContent = clipboardData.getData("text/plain");
        if (ONLY_LINK_REGEX.test(textContent)) {
            return false;
        }
        if (mashoraEditorHtml) {
            const fragment = parseHTML(this.document, mashoraEditorHtml);
    ...
    async onDrop(ev) {
        ...
        const mashoraEditorHtml = ev.dataTransfer.getData("application/vnd.mashora.mashora-editor");
        const fileTransferItems = !mashoraEditorHtml && getImageFiles(dataTransfer);
        const htmlTransferItem = [...dataTransfer.items].find((item) => item.type === "text/html");
        if (fileTransferItems.length || htmlTransferItem || mashoraEditorHtml) {
