...
import { Component, useState, useRef, useEffect, useExternalListener } from "@mashora/owl";
...
export class LinkPopover extends Component {
    ...
    async loadAsyncLinkPreview() {
        ...
        if (!protocol.startsWith("http")) {
            ...
            !new RegExp(`^https?://${session.db}\\.mashora\\.com(/.*)?$`).test(url.origin)
        ) {
    ...
    isAbsoluteURLInCurrentDomain() {
        ...
        // its Mashora instance domain. We just suppose it should be a relative
        // URL (if unexpected behavior, the user can just not enter its Mashora
        // instance domain but its real domain, or opt-out from the domain
        // stripping). Mentioning an .mashora.com domain, especially its own
        // one, is always a bad practice anyway.
        return (
            urlObj.origin === window.location.origin ||
            new RegExp(`^https?://${session.db}\\.mashora\\.com(/.*)?$`).test(urlObj.origin)
        );
    }
}
