...
from mashora import fields, http, tools, _
from mashora.addons.base.models.ir_qweb import keep_query
...
from mashora.addons.website_profile.controllers.main import WebsiteProfile
from mashora.exceptions import AccessError, ValidationError, UserError, MissingError
from mashora.fields import Domain
from mashora.http import request, Response
from mashora.tools import consteq, email_normalize_all
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if not slide.channel_id.is_member:
            if not isinstance(request.session.get('viewed_slides'), dict):
                # Compatibility layer with Mashora 15.0,
                # where `viewed_slides` are stored as `list` in sessions.
                # For performance concerns, `viewed_slides` is changed to a dict,
                # but sessions coming from Mashora 15.0 after an upgrade should still be compatible.
                # This compatibility layer regarding `viewed_slides` must remain from Mashora 16.0 and above,
                # as this is possible to do a jump of multiple versions in one go,
                # and carry the sessions with the upgrade.
                # e.g. upgrade from Mashora 15.0 to 18.0.
