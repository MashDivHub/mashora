...
from mashora.addons.certificate.tools import CertificateAdapter
from mashora.exceptions import UserError
from mashora.tools import float_repr, float_round, frozendict, zeep
...
def _get_zeep_operation(company, operation):
    ...
    if operation == 'registration':
        ...
        zeep_client = client._Client__obj  # get the "real" zeep client from the mashora specific wrapper
...
class L10nEsEdiVerifactuDocument(models.Model):
    ...
    def _check_record_values(self, vals):
        ...
        # We currently do not support cancelling records that are not registered or were registered outside mashora.
        if vals['cancellation'] and not verifactu_registered:
            errors.append(_("The cancelled record is not Veri*Factu registered (inside Mashora)."))
    ...
    def _render_vals_SistemaInformatico(self, vals):
        ...
        render_vals = {
            'SistemaInformatico': {
                'NombreRazon': 'Mashora SA',
                'IDOtro': {
                    'CodigoPais': 'BE',
                    'IDType': '02',  # NIF-IVA
                    'ID': 'BE0477472701',
                },
                'NombreSistemaInformatico': 'Mashora',
                'IdSistemaInformatico': '00',  # identifies Mashora the software as product of Mashora the company
