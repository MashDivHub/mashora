...
import { markRaw, reactive } from "@mashora/owl";
...
export class PosData {
    ...
    async checkConnectivity() {
        try {
            ...
            // if the Mashora server is still accessible.
            //
            // This method also makes it possible to run local tests when no connection is
            // available and an Mashora server is running locally.
            //
    ...
    initializeWebsocket() {
        this.onNotified = getOnNotified(this.bus, mashora.access_token);
    }
    ...
    get databaseName() {
        return `point-of-sale-${mashora.pos_config_id}-${mashora.info?.db}`;
    }
    ...
    async loadInitialData() {
        ...
        if (
            ...
            session?.id !== mashora.pos_session_id ||
            mashora.from_backend
        ) {
            try {
                const limitedLoading = this.isLimitedLoading();
                const serverDate = localData["pos.config"]?.[0]?._data_server_date;
                const lastConfigChange = DateTime.fromSQL(mashora.last_data_change);
                ...
                const data = await this.orm.call(
                    "pos.session",
                    "load_data",
                    [mashora.pos_session_id, PosData.modelToLoad],
                    {
                        context: {
    ...
    async loadFieldsAndRelations() {
        const key = `pos_data_params_${mashora.pos_config_id}`;
            ...
            try {
                if (["product.product", "product.template"].includes(model)) {
                    ...
                    await this.callRelated(
                        ...
                        [mashora.pos_config_id, [[domain, "in", Array.from(ids)]], 0, 0],
                        {
                            context: {
                                load_archived: true,
                            },
    ...
    async loadServerOrders(domain) {
        ...
        const config = this.models["pos.config"].get(mashora.pos_config_id);
        const session = this.models["pos.session"].get(mashora.pos_session_id);
