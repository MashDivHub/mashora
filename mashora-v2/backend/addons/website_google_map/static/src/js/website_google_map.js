...
function initialize_map() {
    ...
    var map = new google.maps.Map(document.getElementById('mashora-google-map'), {
        zoom: 1,
    ...
    async function initializeMarkers(mashoraPartnerData) {
        // Create the markers and cluster them on the map
        if (mashoraPartnerData) { /* mashora_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < mashora_partner_data.counter; i++) {
                const prom = set_marker(mashora_partner_data.partners[i])
    ...
    initializeMarkers(mashora_partner_data);
