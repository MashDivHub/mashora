...
export class TimeDialog extends Component {
    ...
    setup() {
        this.store = useStore();
        this.state = useState({
            mashoraUptimeSeconds: this.store.base.mashora_uptime_seconds,
            systemUptimeSeconds: this.store.base.system_uptime_seconds,
        });
        setInterval(() => {
            this.state.mashoraUptimeSeconds += 1;
    ...
    startDateFromSeconds(uptimeInSeconds) {
        ...
        const mashoraUptimeMs = uptimeInSeconds * 1000;
        return new Date(currentTimeMs - mashoraUptimeMs).toUTCString();
    }
        ...
        <BootstrapDialog identifier="'time-dialog'" btnName="'View Uptime'">
            ...
            <t t-set-slot="body">
                <div class="d-flex flex-column gap-4">
                  <div>
                    <h5>Mashora Service</h5>
                    <div>Running for <b t-out="secondsToHumanReadable(state.mashoraUptimeSeconds)"/></div>
                    <div class="text-secondary">Started at <b t-out="startDateFromSeconds(state.mashoraUptimeSeconds)"/></div>
                  </div>
