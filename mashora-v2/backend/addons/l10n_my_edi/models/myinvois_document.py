...
from mashora import SUPERUSER_ID, api, fields, models, modules
from mashora.exceptions import UserError
from mashora.fields import Domain
from mashora.tools import config, date_utils, split_every
from mashora.tools.image import image_data_uri

from mashora.addons.account.tools import dict_to_xml
...
class MyInvoisDocument(models.Model):
    ...
    In Mashora, a document represent either an invoice, or a group of PoS order.
    """
    ...
    def _myinvois_map_error(self, error):
        ...
        error_map = {
            ...
            "internal_server_error": self.env._(
                "Server error; If the problem persists, please contact the Mashora support."
            ),
            # The proxy user credentials are either incorrect, or Mashora does not have the permission to invoice on their behalf.
            "invalid_tin": self.env._(
                "Please make sure that your company TIN is correct, and that you gave Mashora sufficient permissions on the MyInvois platform."
            ),
            ...
            'action_forbidden': self.env._('Permission to do this action has not been granted. Please ensure that Mashora has sufficient permissions on the MyInvois platform.'),
        }
    ...
    def _myinvois_update_document(self, status, reason):
        """
        This method will try to update the status of a document on the platform, and if needed also the status in Mashora.
    ...
    def _myinvois_set_state(self, state, message=None):
        ...
        # In Mashora, the best way to represent that is by cancelling the invoice.
    ...
    def _myinvois_set_validation_fields(self, validation_result):
        ...
        # Mashora expect a timezone unaware datetime in UTC, so we can safely remove the info without any more work needed.
