...
from mashora.addons.base.models.ir_mail_server import MailDeliveryException
...
from mashora.tests.common import tagged, users, HttpCase
from mashora.tools import is_html_empty, mute_logger, formataddr
...
class TestMessageHelpersRobustness(MailCommon, HttpCase):
    ...
    def setUp(self):
        ...
        with mute_logger('mashora.addons.mail.models.mail_mail'), self.mock_mail_gateway():
    ...
    def test_load_message_failures(self):
        ...
        with contextlib.suppress(Exception), mute_logger('mashora.http', 'mashora.sql_db'):  # suppress logged error due to readonly route doing an update
    ...
    def test_message_link_by_employee(self):
        ...
        with self.subTest(thread_message=thread_message):
            expected_url = self.base_url() + f'/mashora/{thread_message.model}/{thread_message.res_id}?highlight_message_id={thread_message.id}'
...
class TestMessageValues(MailCommon):
    ...
    @mute_logger('mashora.models.unlink', 'mashora.addons.mail.models.models')
    @users('employee')
