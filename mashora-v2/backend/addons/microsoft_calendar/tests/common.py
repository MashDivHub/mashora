...
from mashora.tests.common import HttpCase
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        m = mute_logger('mashora.addons.auth_signup.models.res_users')
        ...
        # simple event values to create a Mashora event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Mashora events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Mashora events
        # -----------------------------------------------------------------------------------------

        self.expected_mashora_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_mashora_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_mashora_recurrency_events_from_outlook = [
            {
    ...
    def sync_mashora_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Mashora. A trick for deactivating the checking
    ...
    def assert_mashora_event(self, mashora_event, expected_values):
        """
        Assert that an Mashora event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        mashora_event_values = mashora_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(mashora_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, mashora_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_mashora_recurrence(self, mashora_recurrence, expected_values):
        """
        Assert that an Mashora recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        mashora_recurrence_values = mashora_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, mashora_recurrence_values.get(k), msg=f"'{k}' mismatch")
