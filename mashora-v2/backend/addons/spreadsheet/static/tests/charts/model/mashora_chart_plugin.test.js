...
import { MashoraBarChart } from "@spreadsheet/chart/mashora_chart/mashora_bar_chart";
import { MashoraChart } from "@spreadsheet/chart/mashora_chart/mashora_chart";
import { MashoraLineChart } from "@spreadsheet/chart/mashora_chart/mashora_line_chart";
...
test("Can add an Mashora Bar chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_bar" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof MashoraBarChart).toBe(true);
...
test("Can add an Mashora Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof MashoraLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an Mashora Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof MashoraChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("Mashora bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "mashora_bar",
...
test("Mashora pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "mashora_pie",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("Mashora line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "mashora_line",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("Data reloaded upon domain update for charts other than pie/bar/line", async () => {
    ...
    updateChart(model, chartId, { type: "mashora_pie" });
    await waitForDataLoaded(model);
...
test("Can import/export an Mashora chart", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "mashora_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("mashora_line");
...
test("Can undo/redo an Mashora chart creation", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "mashora_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_pie" });
    insertChartInSpreadsheet(model, "mashora_bar");
    insertChartInSpreadsheet(model, "mashora_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste Mashora chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof MashoraChart).toBe(true);
...
test("Can cut/paste Mashora chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof MashoraChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Mashora chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load mashora chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert mashora chart from a different model", async () => {
    const { model } = await createModelWithDataSource();
...
test("Mashora chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove mashora chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getMashoraChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getMashoraChartIds().length).toBe(0);
});

test("Mashora chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getMashoraChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getMashoraChartDisplayName(chartId)).toBe("(#1) Mashora Line Chart");
});

test("Every Mashora chart type has a default title", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const figureId = model.getters.getFigureIdFromChartId(chartId);
    const chartTypes = chartRegistry.getKeys().filter((type) => type.startsWith("mashora_"));
    model.dispatch("UPDATE_CHART", {
    ...
    for (const chartType of chartTypes) {
        ...
        expect(model.getters.getMashoraChartDisplayName(chartId)).toBe(`(#1) Mashora ${chartName} Chart`);
    }
});
...
test("Mashora line and bar charts display only horizontal grid lines", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "mashora_line",
    });
    const sheetId = model.getters.getActiveSheetId();
...
test("Can configure the chart datasets", async () => {
    ...
    const { model } = await createSpreadsheetWithChart({
        type: "mashora_bar",
        serverData,
        definition: { type: "mashora_bar", metaData, searchParams, id: "42" },
    });
    await waitForDataLoaded(model);
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("Non-web chart types are using data source in 'bar' mode", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_radar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
