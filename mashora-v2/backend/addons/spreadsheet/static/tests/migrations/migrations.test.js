...
import { load } from "@mashora/o-spreadsheet";
...
test("Mashora formulas are migrated", () => {
    const data = {
        version: 16,
        sheets: [
            {
    ...
    expect(migratedData.sheets[0].cells.A3).toBe(`=MASHORA.FILTER.VALUE.V18("1")`);
    expect(migratedData.sheets[0].cells.A4).toBe(`=MASHORA.LIST("1")`);
    expect(migratedData.sheets[0].cells.A5).toBe(`=MASHORA.LIST.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A6).toBe(`=MASHORA.PIVOT.POSITION("1")`);
...
test("Pivot 'day' arguments are migrated", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=MASHORA.PIVOT("1","21/07/2022")` },
                    A2: { content: `=MASHORA.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=mashora.pivot("1","21/07/2021")` },
                    A4: { content: `=MASHORA.PIVOT("1","test")` },
                    A5: { content: `=mashora.pivot("1","21/07/2021")+"21/07/2021"` },
...
test("Pivot formulas are migrated from 9 to 10", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=MASHORA.PIVOT("1")` },
                    A2: { content: `=MASHORA.PIVOT.HEADER("1")` },
                    A3: { content: `=MASHORA.PIVOT.POSITION("1")` },
                    A4: { content: `=MASHORA.PIVOT.TABLE("1")` },
                    A5: { content: `=mashora.pivot("1")` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1).toBe(`=PIVOT.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A2).toBe(`=PIVOT.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A3).toBe(`=MASHORA.PIVOT.POSITION("1")`);
...
test("Pivot formulas using pivot positions are migrated (11 to 12)", () => {
        ...
        sheets: [
            {
                cells: {
                    A1: {
                        content: `=-PIVOT.VALUE("1","balance","account_id",MASHORA.PIVOT.POSITION("1","account_id",12),"date:quarter","4/"&MASHORA.FILTER.VALUE("Year"))`,
                    },
                    A2: {
                        content: `=PIVOT.HEADER("1","account_id",MASHORA.PIVOT.POSITION("1","account_id",14))`,
                    },
                    A3: { content: `=MASHORA.PIVOT.POSITION("1","account_id",14)` },
                    A4: { content: `=MASHORA.PIVOT.POSITION("1",14)` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1).toBe(
        `=-PIVOT.VALUE("1","balance","#account_id",12,"date:quarter","4/"&MASHORA.FILTER.VALUE.V18("Year"))`
    );
    expect(migratedData.sheets[0].cells.A2).toBe(`=PIVOT.HEADER("1","#account_id",14)`);
    expect(migratedData.sheets[0].cells.A3).toBe(`=MASHORA.PIVOT.POSITION("1","account_id",14)`);
    expect(migratedData.sheets[0].cells.A4).toBe(`=MASHORA.PIVOT.POSITION("1",14)`);
});
...
test("18.5.10: MASHORA.FILTER.VALUE to MASHORA.FILTER.VALUE.V18 in cells", () => {
    const data = {
        version: "18.4.14",
        sheets: [
            {
                cells: {
                    A1: '=MASHORA.FILTER.VALUE("MyFilter")+mashora.filter.value("AnotherFilter")',
                },
            },
        ],
    };
    const migratedData = load(data);
    expect(migratedData.sheets[0].cells.A1).toBe(
        `=MASHORA.FILTER.VALUE.V18("MyFilter")+MASHORA.FILTER.VALUE.V18("AnotherFilter")`
    );
});
