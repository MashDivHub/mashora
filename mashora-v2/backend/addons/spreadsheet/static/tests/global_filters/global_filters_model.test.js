...
import { animationFrame, mockDate, mockTimeZone } from "@mashora/hoot-mock";

import { DispatchResult, Model, helpers, tokenize, constants } from "@mashora/o-spreadsheet";
...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    const chartId = model.getters.getMashoraChartIds()[0];
    fieldMatching = model.getters.getMashoraChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("MASHORA.FILTER.VALUE text filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=MASHORA.FILTER.VALUE("Text Filter")`);
...
test("MASHORA.FILTER.VALUE numeric filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=MASHORA.FILTER.VALUE("Numeric Filter")`);
    await animationFrame();
    expect(getCellValue(model, "A10")).toBe("#ERROR");
    await addGlobalFilter(model, {
        id: "42",
...
test("MASHORA.FILTER.VALUE date filter", async function () {
    mockDate("2022-03-10 00:00:00");
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=MASHORA.FILTER.VALUE("Date Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
...
test("MASHORA.FILTER.VALUE date from/to without values", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(model, "A1", `=MASHORA.FILTER.VALUE("Date Filter")`);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    expect(getEvaluatedCell(model, "B1").value).toBe("");
});

test("MASHORA.FILTER.VALUE date from/to with only from defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=MASHORA.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("MASHORA.FILTER.VALUE date from/to with only to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=MASHORA.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("MASHORA.FILTER.VALUE date from/to with from and to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=MASHORA.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("MASHORA.FILTER.VALUE relation filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=MASHORA.FILTER.VALUE("Relation Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "relation",
...
test("MASHORA.FILTER.VALUE with escaped quotes in the filter label", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "text",
    ...
    setCellContent(model, "A1", '=MASHORA.FILTER.VALUE("my \\"special\\" filter")');
    expect(getCellValue(model, "A1")).toBe("Jean-Jacques");
});

test("MASHORA.FILTER.VALUE formulas are updated when filter label is changed", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(
        model,
        "A10",
        `=MASHORA.FILTER.VALUE("Cuillère") & MASHORA.FILTER.VALUE( "Cuillère" )`
    );
    const [filter] = model.getters.getGlobalFilters();
    const newFilter = {
        ...filter,
    ...
    expect(getCellFormula(model, "A10")).toBe(
        `=MASHORA.FILTER.VALUE("Interprete") & MASHORA.FILTER.VALUE("Interprete")`
    );
});
...
test("field matching is removed when list is deleted", async function () {
    ...
    model.dispatch("REMOVE_MASHORA_LIST", { listId });
...
test("field matching is removed when an Mashora chart is deleted", async function () {
    ...
    expect(model.getters.getMashoraChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", {
        figureId: model.getters.getFigureIdFromChartId(chartId),
        sheetId,
    });
    expect(() => model.getters.getMashoraChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
...
test("Updating the pivot domain should keep the global filter domain", async () => {
    ...
    model.dispatch("UPDATE_MASHORA_PIVOT_DOMAIN", {
        pivotId,
        domain: [["foo", "in", [55]]],
    });
...
test("Updating a non-mashora pivot should not crash on global filter", async () => {
    const grid = {
        A1: "Customer",
        B1: "Price",
        C1: `=PIVOT(1)`,
