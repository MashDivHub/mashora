 ...
 * @typedef {import("@mashora/o-spreadsheet").FPayload} FPayload
 */
...
// MASHORA.FILTER.VALUE

const MASHORA_FILTER_VALUE = /** @satisfies {CustomFunctionDescription} */ ({
...
// MASHORA.FILTER.VALUE.V18

const MASHORA_FILTER_VALUE_V18 = /** @satisfies {CustomFunctionDescription} */ ({
    description: _t("Compatibility version of MASHORA.FILTER.VALUE for v18 spreadsheets. Required for date filters. Optional for others."),
    args: [arg("filter_name (string)", _t("The label of the filter whose value to return."))],
    category: "Mashora",
    hidden: true,
    compute: function (filterName) {
        const filter = this.getters.getGlobalFilterByName(toString(filterName, this.locale));
        const value = this["MASHORA.FILTER.VALUE"](filterName);
        if (filter?.type === "relation") {
            ...
            const result =  this.mashoraDataProvider.serverData.get(filter.modelName, "web_search_read", [
                [["id", "in", ids]],
                { display_name: {} },
            ])
...
functionRegistry
    .add("MASHORA.FILTER.VALUE", MASHORA_FILTER_VALUE)
    .add("MASHORA.FILTER.VALUE.V18", MASHORA_FILTER_VALUE_V18);
