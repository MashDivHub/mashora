...
import { registries, helpers, constants, EvaluationError } from "@mashora/o-spreadsheet";
...
function boundedMashoraNumberDateAdapter(lower, upper) {
    return {
...
const mashoraHourNumberAdapter = {
    normalizeServerValue(groupBy, field, readGroupResult) {
        return Number(readGroupResult[groupBy]);
    },
    increment(normalizedValue, step) {
...
const mashoraMinuteNumberAdapter = {
    normalizeServerValue(groupBy, field, readGroupResult) {
        return Number(readGroupResult[groupBy]);
    },
    increment(normalizedValue, step) {
...
const mashoraSecondNumberAdapter = {
    normalizeServerValue(groupBy, field, readGroupResult) {
        return Number(readGroupResult[groupBy]);
    },
    increment(normalizedValue, step) {
...
pivotTimeAdapterRegistry.add("week", falseHandlerDecorator(mashoraWeekAdapter));
pivotTimeAdapterRegistry.add("quarter", falseHandlerDecorator(mashoraQuarterAdapter));

extendSpreadsheetAdapter("day", mashoraDayAdapter);
extendSpreadsheetAdapter("year", mashoraYearAdapter);
extendSpreadsheetAdapter("day_of_month", boundedMashoraNumberDateAdapter(1, 31));
extendSpreadsheetAdapter("iso_week_number", boundedMashoraNumberDateAdapter(0, 54));
extendSpreadsheetAdapter("month_number", boundedMashoraNumberDateAdapter(1, 12));
extendSpreadsheetAdapter("quarter_number", boundedMashoraNumberDateAdapter(1, 4));
extendSpreadsheetAdapter("day_of_week", mashoraDayOfWeekAdapter);
extendSpreadsheetAdapter("hour_number", mashoraHourNumberAdapter);
extendSpreadsheetAdapter("minute_number", mashoraMinuteNumberAdapter);
extendSpreadsheetAdapter("second_number", mashoraSecondNumberAdapter);
extendSpreadsheetAdapter("month", mashoraMonthAdapter);
