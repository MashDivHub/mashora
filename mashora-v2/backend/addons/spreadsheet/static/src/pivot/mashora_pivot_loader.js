...
import { EvaluationError, CellErrorType } from "@mashora/o-spreadsheet";
...
export class MashoraPivotLoader {
    /**
     * @param {MashoraDataProvider} mashoraDataProvider
     * @param {Function} load Function to fetch data
     */
    constructor(mashoraDataProvider, load) {
        /** @private @type {MashoraDataProvider} */
        this.mashoraDataProvider = mashoraDataProvider;
        /** @protected */
        this.loadFn = load;
    ...
    async load(options) {
        if (options && options.reload) {
            this.mashoraDataProvider.cancelPromise(this.loadPromise);
            this.loadPromise = undefined;
        }
        if (!this.loadPromise) {
            this.isFullyLoaded = false;
            ...
            await this.mashoraDataProvider.notifyWhenPromiseResolves(this.loadPromise);
        }
     ...
     * @returns {Promise<MashoraFields>} Fields of the model
     */
    async getFields(model) {
        return getFields(this.mashoraDataProvider.fieldService, model);
    }
    /**
     * @param {string} model Technical name of the model
    ...
    async getModelLabel(model) {
        const result = await this.mashoraDataProvider.orm
            .cache({ type: "disk" })
