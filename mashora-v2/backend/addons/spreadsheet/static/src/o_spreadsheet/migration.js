...
const MAP_V1 = {
    PIVOT: "MASHORA.PIVOT",
    "PIVOT.HEADER": "MASHORA.PIVOT.HEADER",
    "PIVOT.POSITION": "MASHORA.PIVOT.POSITION",
    "FILTER.VALUE": "MASHORA.FILTER.VALUE",
    LIST: "MASHORA.LIST",
    "LIST.HEADER": "MASHORA.LIST.HEADER",
};

const MAP_FN_NAMES_V10 = {
    "MASHORA.PIVOT": "PIVOT.VALUE",
    "MASHORA.PIVOT.HEADER": "PIVOT.HEADER",
    "MASHORA.PIVOT.TABLE": "PIVOT",
};
...
migrationStepRegistry.add("17.3.1", {
    migrate(data) {
        return migrateMashoraData(data);
    },
});
migrationStepRegistry.add("18.1.2", {
    migrate(data) {
        const version = data.mashoraVersion || 0;
        if (version < 13) {
    ...
    migrate(data) {
        for (const sheet of data.sheets || []) {
            for (const figure of sheet.figures || []) {
                if (
                    figure.tag === "chart" &&
                    ["mashora_bar", "mashora_line", "mashora_pie"].includes(figure.data.type) &&
...
migrationStepRegistry.add("18.5.10", {
    migrate(data) {
        ...
        const re = /MASHORA\.FILTER\.VALUE/gi
        for (const sheet of data.sheets || []) {
            for (const xc in sheet.cells || {}) {
                const content = sheet.cells[xc];
                if (re.test(content)) {
                    sheet.cells[xc] = content.replaceAll(re, "MASHORA.FILTER.VALUE.V18");
                }
            }
        }
        return data;
...
function migrateMashoraData(data) {
    const version = data.mashoraVersion || 0;
    if (version < 1) {
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["MASHORA.PIVOT", "MASHORA.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
/**
 * Migrate the pivot data to add the type, by default "MASHORA". And replace the
...
function migrate11to12(data) {
    // remove the calls to MASHORA.PIVOT.POSITION and replace
    ...
    for (const sheet of data.sheets || []) {
        for (const xc in sheet.cells || []) {
            ...
            if (
                ...
                cell.content.includes("MASHORA.PIVOT.POSITION")
            ) {
                const tokens = tokenize(cell.content);
                /* given that mashora.pivot.position is automatically set, we know that:
                1) it is always on the form of MASHORA.PIVOT.POSITION(1, ...)
                2) it is always preceded by a dimension of a pivot or header, inside another pivot formula
                3) there is only one mashora.pivot.position per cell
                4) mashora.pivot.position can only exist after the 3rd token and needs at least 7 tokens to be valid*/
                for (let i = 2; i < tokens.length - 7; i++) {
                    const token = tokens[i];
                    if (
                        token.type === "SYMBOL" &&
                        token.value.toUpperCase() === "MASHORA.PIVOT.POSITION"
                    ) {
                        const order = tokens[i + 6];
                        tokens[i - 2].value = '"#' + tokens[i - 2].value.slice(1); // "dimension" becomes "#dimension"
                        tokens.splice(i, 7); // remove "MASHORA.PIVOT.POSITION", "(", "1", ",", "dimension", ", ", order
                        // tokens[i-1] is the comma before mashora.pivot.position
                        tokens[i] = order;
                        cell.content = tokensToString(tokens);
                    }
                }
...
/**
 * Change pivot sortedColumn of the mashora's pivot model to the sortedColumn of o-spreadsheet
 */
