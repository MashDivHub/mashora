...
import { parse, helpers, iterateAstNodes } from "@mashora/o-spreadsheet";
import { isLoadingError } from "@spreadsheet/o_spreadsheet/errors";
import { MashoraSpreadsheetModel } from "@spreadsheet/model";
...
import {
    ...
    isIrMenuXmlUrl,
} from "@spreadsheet/ir_ui_menu/mashora_menu_link_cell";
...
export function createSpreadsheetModel({ env, data, revisions }) {
    const mashoraDataProvider = new MashoraDataProvider(env);
    const model = new MashoraSpreadsheetModel(data, { custom: { env, mashoraDataProvider } }, revisions);
    return model;
}

/**
 * @param {MashoraSpreadsheetModel} model
 */
export async function waitForMashoraSources(model) {
    ...
    promises.push(
        ...model.getters
            .getPivotIds()
            .filter((pivotId) => model.getters.getPivotCoreDefinition(pivotId).type === "MASHORA")
...
export async function waitForDataLoaded(model) {
    await waitForMashoraSources(model);
    const mashoraDataProvider = model.config.custom.mashoraDataProvider;
    if (!mashoraDataProvider) {
        return;
    }
    await new Promise((resolve, reject) => {
        function check() {
            ...
            if (isLoaded(model)) {
                mashoraDataProvider.removeEventListener("data-source-updated", check);
                resolve();
            }
        }
        mashoraDataProvider.addEventListener("data-source-updated", check);
        check();
    });
}

function containsLinkToMashora(link) {
    if (link && link.url) {
        return (
 ...
 */
export async function freezeMashoraData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const [xc, content] of Object.entries(sheet.cells)) {
            ...
            if (containsMashoraFunction(content)) {
                const pivotId = model.getters.getPivotIdFromPosition(position);
                if (pivotId && model.getters.getPivotCoreDefinition(pivotId).type !== "MASHORA") {
                    continue;
                }
            ...
            if (containsLinkToMashora(evaluatedCell.link)) {
        ...
        for (const figure of sheet.figures) {
            if (
                figure.tag === "chart" &&
                (figure.data.type.startsWith("mashora_") || figure.data.type === "geo")
            ) {
                const img = mashoraChartToImage(model, figure, figure.data.chartId);
                figure.tag = "image";
                figure.data = {
                    path: img,
                ...
                const hasImageChart = figure.data.items.some((item) => {
                    ...
                    return (
                        chartDefinition.type.startsWith("mashora_") || chartDefinition.type === "geo"
                    );
                });
                if (hasImageChart) {
                    ...
                    figure.tag = "image";
                    figure.data = {
                        path: mashoraChartToImage(model, figure, chartId),
                        size: { width: figure.width, height: figure.height },
                    };
                }
            }
    ...
    if (data.pivots) {
        data.pivots = Object.fromEntries(
            Object.entries(data.pivots).filter(([id, def]) => def.type !== "MASHORA")
        );
    }
    data.lists = {};
 ...
 * @returns {boolean}
 */
function containsMashoraFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
        (!content.toUpperCase().includes("MASHORA.") &&
    ...
    try {
        ...
        return iterateAstNodes(ast).some(
            (ast) =>
                ast.type === "FUNCALL" &&
                (ast.value.toUpperCase().startsWith("MASHORA.") ||
 ...
 * @returns {string}
 */
function mashoraChartToImage(model, figure, chartId) {
