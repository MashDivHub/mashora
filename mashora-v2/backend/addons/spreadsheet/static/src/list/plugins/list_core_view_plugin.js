...
import { MashoraCoreViewPlugin } from "@spreadsheet/plugins";
...
export class ListCoreViewPlugin extends MashoraCoreViewPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "SET_GLOBAL_FILTER_VALUE":
                this._addDomains();
                break;
            case "UPDATE_MASHORA_LIST":
            case "UPDATE_MASHORA_LIST_DOMAIN": {
            ...
            case "REDO": {
                ...
                const updateCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_MASHORA_LIST_DOMAIN" ||
                        cmd.type === "UPDATE_MASHORA_LIST" ||
                        cmd.type === "INSERT_MASHORA_LIST"
                );
     ...
     */
    _refreshMashoraLists() {
        for (const listId of this.getters.getListIds()) {
            this._refreshMashoraList(listId);
        }
    }
    ...
    getListFieldFromPosition(position) {
        ...
        const fieldArg = functionName === "MASHORA.LIST.HEADER" ? args[1] : args[2];
    ...
    isSortableListHeader(position) {
        ...
        const dataSource = this.getters.getListDataSource(listId);
        return (
            functionName === "MASHORA.LIST.HEADER" &&
