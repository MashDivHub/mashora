...
export function onMashoraChartItemClick(getters, chart) {
    return navigateInMashoraMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onWaterfallMashoraChartItemClick(getters, chart) {
    return navigateInMashoraMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onGeoMashoraChartItemClick(getters, chart) {
    return navigateInMashoraMenuOnClick(getters, chart, (chartJsItem) => {
...
export function onSunburstMashoraChartItemClick(getters, chart) {
    return navigateInMashoraMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
...
export function onTreemapMashoraChartItemClick(getters, chart) {
    return navigateInMashoraMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
        const rawItem = chartJSChart.data.datasets[datasetIndex].data[index];
...
function navigateInMashoraMenuOnClick(getters, chart, getDomainFromChartItem) {
    return async (event, items, chartJSChart) => {
        const env = getters.getMashoraEnv();
        const { datasets, labels } = chart.dataSource.getData();
...
export function onMashoraChartItemHover() {
    return (event, items) => {
        if (items.length > 0) {
...
export function onGeoMashoraChartItemHover() {
    return (event, items) => {
        if (!items.length) {
            event.native.target.style.cursor = "";
            return;
...
export async function navigateToMashoraMenu(menu, actionService, notificationService, newWindow) {
    if (!menu) {
