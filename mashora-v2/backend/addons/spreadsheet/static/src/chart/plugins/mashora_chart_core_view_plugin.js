...
export class MashoraChartCoreViewPlugin extends MashoraUIPlugin {
    static getters = /** @type {const} */ (["getChartDataSource", "getMashoraEnv"]);
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getMashoraChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getMashoraChartIds().includes(cmd.chartId)) {
                        continue;
                    }
                    if (this._shouldReloadDataSource(cmd.chartId, cmd.definition)) {
                        this._resetChartDataSource(cmd.chartId);
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getMashoraChartDataSourceId(chartId);
    ...
    _getMashoraChartDataSourceId(chartId) {
     ...
     */
    _refreshMashoraCharts() {
        for (const chartId of this.getters.getMashoraChartIds()) {
            this._refreshMashoraChart(chartId);
        }
    }
