...
from mashora.tools.view_validation import get_domain_value_names


markdown_link_regex = r"^\[([^\[]+)\]\((.+)\)$"

xml_id_url_prefix = "mashora://ir_menu_xml_id/"

mashora_view_link_prefix = "mashora://view/"


def mashora_charts(data):
    """returns all mashora chart definitions in the spreadsheet"""
    figures = []
    for sheet in data.get("sheets", []):
        for figure in sheet.get("figures", []):
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("mashora_"):
                figures.append(dict(figure["data"], id=figure["id"]))
            elif figure["tag"] == "carousel":
                figures.extend(get_mashora_charts_from_carousel(figure["data"]))
    return figures


def get_mashora_charts_from_carousel(carousel):
    charts = []
    for chart_id, chart in carousel["chartDefinitions"].items():
        if chart["type"].startswith("mashora_"):
...
def mashora_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like mashora://view/{... view data...}
    """
    return [
        json.loads(url[len(mashora_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(mashora_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = mashora_charts(data)
    if "mashoraVersion" in data and data["mashoraVersion"] < 5:
        ...
        for pivot in data.get("pivots", {}).values():
            if pivot.get("type", "MASHORA") == "MASHORA":
                model = pivot["model"]
...
def fields_in_spreadsheet(data):
    ...
    mashora_pivots = (pivot for pivot in data.get("pivots", dict()).values() if pivot.get("type", "MASHORA") == "MASHORA")
    all_fields = chain(
        extract_fields(list_fields, data.get("lists", dict()).values()).items(),
        extract_fields(pivot_fields, mashora_pivots).items(),
        extract_fields(chart_fields, mashora_charts(data)).items(),
        extract_fields(mashora_view_fields, mashora_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartMashoraMenusReferences", {}).values()) | {
