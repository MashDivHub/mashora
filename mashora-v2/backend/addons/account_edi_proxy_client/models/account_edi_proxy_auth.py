...
class MashoraEdiProxyAuth(requests.auth.AuthBase):
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'mashora-edi-client-id': self.id_client,
            'mashora-edi-timestamp': timestamp,
        })
        message = self.__get_payload(request, timestamp)

        if self.auth_type == 'asymmetric' and self.private_key:
            request.headers.update({
                'mashora-edi-signature': self.__sign_with_private_key(message),
                'mashora-edi-signature-type': 'asymmetric'
            })
        elif self.refresh_token:
            request.headers.update({
                'mashora-edi-signature': self.__sign_request_with_token(message),
                'mashora-edi-signature-type': 'hmac'
            })
