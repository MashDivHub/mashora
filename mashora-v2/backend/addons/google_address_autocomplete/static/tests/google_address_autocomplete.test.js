...
import { runAllTimers } from "@mashora/hoot-mock";
import {
    contains,
    defineModels,
    fields,
...
test("correctly fill all standard fields", async () => {
    ...
    currentInput = "mashora farm 2";
    await contains(".o_field_widget[name='street'] input").edit("mashora farm 2", { confirm: false });
    await runAllTimers();
    ...
    currentInput = "mashora farm 3";
    await contains(".o_field_widget[name='street'] input").edit("mashora farm 3", { confirm: false });
    await runAllTimers();
    expect.verifySteps(["/autocomplete/address"]);
...
test("fills current field with values of unknown ones", async () => {
    ...
    await contains(".o_field_widget[name='some_char'] input").edit("mashora farm 2", {
        confirm: false,
    });
    await runAllTimers();
    await contains(
...
test("typing in input should make form dirty", async () => {
    ...
    expect.verifySteps([{ street: "mashora farm 3" }]);
});
...
test("select a value, then empty the input and save", async () => {
    ...
    await contains(".o_field_widget[name=street] input").edit("mashora farm 2", {
        confirm: false,
    });
    await runAllTimers();
    await contains(
