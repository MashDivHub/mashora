...
while true; do
    ...
    if ! systemctl is-active --quiet mashora.service; then
        # Mashora service is not running: Red ON, Green OFF
        led_constant "$RED_LED"
        led_off "$GREEN_LED"
        continue
    fi

    if grep -q "remote_server" /home/pi/mashora.conf 2>/dev/null; then
