...
from mashora import _, api, Command, fields, models, modules, tools
from mashora.exceptions import UserError
from mashora.http import request
from mashora.tools import email_normalize, str2bool
...
class ResUsers(models.Model):
    ...
    notification_type = fields.Selection([
        ...
        help="Policy on how to handle Chatter notifications:\n"
             ...
             "- In Mashora: notifications appear in your Mashora Inbox")
    ...
    _notification_type = models.Constraint(
        ...
        'Only internal user can receive notifications in Mashora',
    )
    ...
    def _init_store_data(self, store: Store):
        ...
        # sudo: res.partner - exposing MashoraBot data is considered acceptable
        mashorabot = self.env.ref("base.partner_root").sudo()
        if not self.env.user._is_public():
            mashorabot = mashorabot.with_prefetch((mashorabot + self.env.user.partner_id).ids)
        store.add_global_values(
            ...
            mashorabot=Store.One(mashorabot),
        )
        if not self.env.user._is_public():
