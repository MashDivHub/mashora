...
export class OutOfFocusService {
    ...
    async notify(message, thread) {
        ...
        let icon = "/mail/static/src/img/mashorabot_transparent.png";
        if (!author) {
    ...
    async sendNotification({ message, sound = true, title, type, icon }) {
        ...
        try {
            ...
            if (error.message.includes("ServiceWorkerRegistration")) {
                this.sendMashoraNotification(message, { sound, title, type });
            } else {
                throw error;
            }
        }
     ...
     */
    async sendMashoraNotification(message, options) {
        const { sound } = options;
