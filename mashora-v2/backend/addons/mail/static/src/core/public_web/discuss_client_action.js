...
import { Component, onMounted, onWillStart, onWillUnmount, onWillUpdateProps } from "@mashora/owl";
...
export class DiscussClientAction extends Component {
    ...
    async restoreDiscussThread(props) {
        ...
        if (!parsedActiveId) {
            ...
            const mashorabotChat = this.store.mashorabot?.searchChat();
            const selfMember = mashorabotChat?.self_member_id;
            if (mashorabotChat && selfMember?.is_pinned && !selfMember.seen_message_id) {
                mashorabotChat.setAsDiscussThread(false);
            }
            return;
        }
