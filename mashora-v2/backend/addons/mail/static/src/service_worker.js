...
let db;
const unread_store = new Store("mashora-mail-unread-db", "mashora-mail-unread-store");
 ...
 * @param {Object} param1
 * @param {string} [param1.action] mashora client action
...
async function openDiscussChannel(channelId, { action, joinCall = false, source } = {}) {
    const discussURLRegexes = [new RegExp("/mashora/discuss")];
    if (action) {
        discussURLRegexes.push(
            new RegExp(`/mashora/\\d+/action-${action}`),
            new RegExp(`/mashora/action-${action}`)
        );
    }
    let targetClient;
    ...
    if (action) {
        const url = new URL(`/mashora/action-${action}`, location.origin);
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            event.waitUntil(clients.openWindow(`/mashora/${modelPath}/${res_id}`));
        }
    }
});
