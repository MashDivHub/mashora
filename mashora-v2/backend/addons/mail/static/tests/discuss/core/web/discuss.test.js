...
import { describe, test } from "@mashora/hoot";
import {
    asyncStep,
    Command,
    onRpc,
...
test("Auto-open MashoraBot chat when opening discuss for the first time", async () => {
    // Mashorabot chat has onboarding for using Discuss app.
    // We assume pinned mashorabot chat without any message seen means user just started using Discuss app.
    const pyEnv = await startServer();
    ...
    await start();
    await openDiscuss();
    await contains(".o-mail-DiscussContent-threadName", { value: "MashoraBot" });
});
