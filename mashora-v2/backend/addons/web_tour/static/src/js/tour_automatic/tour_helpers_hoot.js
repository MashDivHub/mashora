import * as hoot from "@mashora/hoot-dom";
...
patch(TourHelpers.prototype, {
     ...
     * @param {Selector} selector
     * @param {import("@mashora/hoot-dom").PointerOptions} options
     * @example
    ...
    async editor(text, selector) {
        const element = this._get_action_element(selector);
        const InEditor = Boolean(element.closest(".mashora-editor-editable"));
        if (!InEditor) {
