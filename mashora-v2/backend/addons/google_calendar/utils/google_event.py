...
class GoogleEvent(abc.Set):
    ...
    Inspired by Mashora recordset, one instance can be a single Google event or a
    ...
    def mashora_id(self, env):
        self.mashora_ids(env)  # load ids
        return self._mashora_id

    def _meta_mashora_id(self, dbname):
        """Returns the Mashora id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_mashora_id' % dbname)
        if o_id:
            return int(o_id)

    def mashora_ids(self, env):
        ids = tuple(e._mashora_id for e in self if e._mashora_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_mashora_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_mashora_ids_from_metadata(env, model)

        return tuple(e._mashora_id for e in self)

    def _load_mashora_ids_from_metadata(self, env, model):
        unsure_mashora_ids = tuple(e._meta_mashora_id(env.cr.dbname) for e in self)
        mashora_events = model.browse(_id for _id in unsure_mashora_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Mashora id.
        # Therefore, we only consider Mashora records without google id when trying
        # to match events.
        o_ids = mashora_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            mashora_id = e._meta_mashora_id(env.cr.dbname)
            if mashora_id in o_ids:
                e._events[e.id]['_mashora_id'] = mashora_id

    def _load_mashora_ids_from_db(self, env, model):
        mashora_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in mashora_events}  # {google_id: mashora_id}
        existing_google_ids = mashora_events.mapped('google_id')
        for e in self:
            mashora_id = mapping.get(e.id)
            if mashora_id:
                e._events[e.id]['_mashora_id'] = mashora_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Mashora.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Mashora (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Mashora) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Mashora several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_mashora_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.mashora_ids(env)
        events.mashora_ids(env)

        return self.filter(lambda e: e._mashora_id)
    ...
    def get_mashora_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.mashora_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.mashora_id(self.env)).base_event_id
