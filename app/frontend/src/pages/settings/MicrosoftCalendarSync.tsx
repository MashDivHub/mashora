...
export default function MicrosoftCalendarSync() {
  const { data, isLoading } = useQuery({
    queryKey: ['settings', 'microsoft-calendar'],
    queryFn: async () => {
      try {
        const { data } = await erpClient.raw.post('/model/res.config.settings', {
          fields: ['id', 'microsoft_outlook_client_identifier', 'microsoft_outlook_client_secret'],
          limit: 1,
          order: 'id desc',
        })
  ...
  const clientId: string | undefined = data?.microsoft_outlook_client_identifier
  const clientSecret: string | undefined = data?.microsoft_outlook_client_secret
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm text-muted-foreground mb-1.5">
                Synchronize your Microsoft Outlook calendar events with Mashora calendar
              </p>
              <span
      ...
      <div className="rounded-2xl border border-border/30 bg-card/50 p-6">
        <div className="flex items-start gap-3">
          ...
          <p className="text-sm text-muted-foreground">
            Configure Microsoft OAuth credentials in{' '}
