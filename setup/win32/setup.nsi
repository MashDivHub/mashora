...
!ifndef SERVICENAME
	!define SERVICENAME 'mashora-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\mashorabuild'
!endif

!define PRODUCT_NAME "Mashora"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Mashora Server ${VERSION}"
...
OutFile "${TOOLSDIR}\server\mashora_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\mashora-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\mashora-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\mashora-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\mashora-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.mashora.com/page/contactus"
...
; English
LangString DESC_Mashora_Server ${LANG_ENGLISH} "Install the Mashora Server with all the Mashora standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Mashora."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Mashora for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Mashora Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "Mashora Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "Mashora Server Only"
LangString TITLE_Mashora_Server ${LANG_ENGLISH} "Mashora Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Mashora"
LangString UnsafeDirText ${LANG_ENGLISH} "Installing outside of $PROGRAMFILES64 is not recommended.$\nDo you want to continue ?"

; French
LangString DESC_Mashora_Server ${LANG_FRENCH} "Installation du Serveur Mashora avec tous les modules Mashora standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Mashora."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Mashora pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Mashora"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur Mashora Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur Mashora"
LangString TITLE_Mashora_Server ${LANG_FRENCH} "Serveur Mashora"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Mashora"
...
Section $(TITLE_Mashora_Server) SectionMashora_Server
    SectionIn 1 2
    ...
    # If there is a previous install of the Mashora Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\mashora.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\mashora.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\mashora.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\mashora.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\mashora.conf" "options" "addons_path" "$INSTDIR\server\mashora\addons"
    WriteIniStr "$INSTDIR\server\mashora.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\mashora.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\mashora.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\mashora-bin" --stop-after-init -c "$INSTDIR\server\mashora.conf" --logfile "$INSTDIR\server\mashora.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\mashora-bin\" -c "\"$INSTDIR\server\mashora.conf\"'

    Call RestartMashoraService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_Mashora" \
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@mashora.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.mashora.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@mashora.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionMashora_Server} $(DESC_Mashora_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Mashora ${VERSION}"
...
Function ComponentLeave
    SectionGetFlags ${SectionMashora_Server} $0
...
Function RestartMashoraService
    DetailPrint "Restarting Mashora Service"
