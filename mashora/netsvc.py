...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "mashora.netsvc.log is deprecated starting Mashora 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('mashora.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('mashora.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='mashora.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("MASHORA_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'mashora.http.rpc.request:INFO',
    'mashora.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['mashora:DEBUG', 'mashora.sql_db:INFO', 'mashora.http.rpc:DEBUG'],
    'debug_rpc': ['mashora:DEBUG', 'mashora.sql_db:INFO', 'mashora.http.rpc.request:DEBUG'],
    'debug': ['mashora:DEBUG', 'mashora.sql_db:INFO'],
    'debug_sql': ['mashora.sql_db:DEBUG'],
    'info': [],
    'runbot': ['mashora:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['mashora:WARNING', 'werkzeug:WARNING'],
    'error': ['mashora:ERROR', 'werkzeug:ERROR'],
    'critical': ['mashora:CRITICAL', 'werkzeug:CRITICAL'],
}
...
def showwarning_with_traceback(message, category, filename, lineno, file=None, line=None):
    ...
    for frame in traceback.extract_stack():
        if frame.name == '__call__' and frame.filename.endswith('/mashora/http.py'):
