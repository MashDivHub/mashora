...
import mashora.cli
from mashora.modules import initialize_sys_path, load_script
...
def load_internal_commands():
    """ Load ``commands`` from ``mashora.cli`` """
    for path in mashora.cli.__path__:
        for module in Path(path).iterdir():
            if module.suffix != '.py':
                continue
            __import__(f'mashora.cli.{module.stem}')
    ...
    for path in mashora.addons.__path__:
        for fullpath in Path(path).glob(f'*/cli/{command}.py'):
            if (found_command := fullpath.stem) and Command.is_valid_name(found_command):
                # loading as mashora.cli and not mashora.addons.{module}.cli
                # so it doesn't load mashora.addons.{module}.__init__
                mapping[f'mashora.cli.{found_command}'] = fullpath 
...
def find_command(name: str) -> Command | None:
    ...
    # import from mashora.cli
    with contextlib.suppress(ImportError):
        __import__(f'mashora.cli.{name}')
        return commands[name]

    # import from mashora.addons.*.cli
...
def main():
    ...
    if command := find_command(command_name):
        mashora.cli.COMMAND = command_name
        command().run(args)
    else:
        message = (
