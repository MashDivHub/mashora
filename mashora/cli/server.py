...
from mashora.release import author as __author__  # noqa: F401
from mashora.release import version as __version__  # noqa: F401
from mashora.service import server
from mashora.tools import config

from . import Command

# Also use the `mashora` logger for the main script.
_logger = logging.getLogger('mashora')
...
def report_configuration():
    ...
    import mashora.release  # noqa: PLC0415
    _logger.info("Mashora version %s", mashora.release.version)
    if os.path.isfile(config['config']):
        _logger.info("Using configuration file at %s", config['config'])
    _logger.info('addons paths: %s', mashora.addons.__path__)
    ...
    if sys.version_info[:2] > mashora.release.MAX_PY_VERSION:
        _logger.warning("Python %s is not officially supported, please use Python %s instead",
            '.'.join(map(str, sys.version_info[:2])),
            '.'.join(map(str, mashora.release.MAX_PY_VERSION))
        )
...
def setup_pid_file():
    ...
    This function assumes the configuration has been initialized.
    """
    import mashora  # for evented  # noqa: PLC0415
    if not mashora.evented and config['pidfile']:
        pid = os.getpid()
...
def main(args):
    ...
    for db_name in config['db_name']:
        from mashora.service import db  # noqa: PLC0415
        try:
...
class Server(Command):
    """Start the mashora server (default command)"""
