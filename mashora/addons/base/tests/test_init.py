...
class TestInit(BaseCase):
    ...
    def mashora_modules_to_test(self):
        import mashora.cli  # noqa: PLC0415
        for path in (*mashora.__path__, *mashora.cli.__path__):
            parent = Path(path)
            for module in parent.iterdir():
                if (module.is_dir() or module.suffix == '.py') and '__' not in module.name:
                    if parent.name == 'mashora':
                        yield f"mashora.{module.stem}"
                    else:
                        yield f"mashora.{parent.name}.{module.stem}"
    ...
    def test_import(self):
        ...
        for module in sorted(self.mashora_modules_to_test()):
