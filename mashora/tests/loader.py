...
from .suite import MashoraSuite
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'mashora.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"mashora.upgrade.{module}",
        f"mashora.addons.{module}.migrations",
        f"mashora.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return MashoraSuite(sorted(tests, key=lambda t: getattr(t, 'test_sequence', 0)))
...
def run_suite(suite, global_report=None):
    ...
    results = MashoraTestResult(global_report=global_report)
    suite(results)
