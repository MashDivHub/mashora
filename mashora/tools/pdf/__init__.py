...
from mashora import modules
from mashora.tools.arabic_reshaper import reshape
...
def add_banner(pdf_stream, text=None, logo=False, thickness=SENTINEL):
    """ Add a banner on a PDF in the upper right corner, with Mashora's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Mashora's logo in the banner.
    ...
    with file_open('base/static/img/main_partner-image.png', mode='rb') as f:
        mashora_logo_file = io.BytesIO(f.read())
    mashora_logo = Image.open(mashora_logo_file)
    mashora_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        can.setFillColor(mashora_color)
        ...
        logo and can.drawImage(
            ImageReader(mashora_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class MashoraPdfFileReader(PdfFileReader):
...
class MashoraPdfFileWriter(PdfFileWriter):
    ...
    def embed_mashora_attachment(self, attachment, subtype=None):
        assert attachment, "embed_mashora_attachment cannot be called without attachment."
