...
async def get_vehicles(params: VehicleListParams | None = None, user: CurrentUser = Depends(get_current_user)):
...
async def get_vehicle_detail(vehicle_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def update_vehicle_detail(vehicle_id: int, body: VehicleUpdate, user: CurrentUser = Depends(get_current_user)):
...
async def create_vehicle(body: VehicleCreate, user: CurrentUser = Depends(get_current_user)):
...
async def get_vehicle_costs(vehicle_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def create_cost(body: VehicleCostCreate, user: CurrentUser = Depends(get_current_user)):
...
async def get_repairs(params: RepairListParams | None = None, user: CurrentUser = Depends(get_current_user)):
...
async def get_repair_detail(repair_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def update_repair_detail(repair_id: int, body: RepairUpdate, user: CurrentUser = Depends(get_current_user)):
...
async def create_repair(body: RepairCreate, user: CurrentUser = Depends(get_current_user)):
...
async def confirm_repair(repair_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def start_repair(repair_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def complete_repair(repair_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def cancel_repair(repair_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_productions(params: ProductionListParams | None = None, user: CurrentUser = Depends(get_current_user)):
...
async def get_production_detail(production_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def update_production_detail(production_id: int, body: ProductionUpdate, user: CurrentUser = Depends(get_current_user)):
...
async def create_production(body: ProductionCreate, user: CurrentUser = Depends(get_current_user)):
...
async def confirm_production(production_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def start_production(production_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def produce_production(production_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def cancel_production(production_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_boms(params: BomListParams | None = None, user: CurrentUser = Depends(get_current_user)):
...
async def get_bom_detail(bom_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_events(params: EventListParams | None = None, user: CurrentUser = Depends(get_current_user)):
    p = params or EventListParams()
    return await orm_call(list_events, params=p.model_dump(), uid=_uid(user), context=_ctx(user))

@router.get("/events/{event_id}")
async def get_event_detail(event_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def update_event_detail(event_id: int, body: EventUpdate, user: CurrentUser = Depends(get_current_user)):
...
async def confirm_event(event_id: int, user: CurrentUser = Depends(get_current_user)):
    return await orm_call(event_action, event_id=event_id, action="button_confirm", uid=_uid(user), context=_ctx(user))

@router.post("/events/{event_id}/cancel")
async def cancel_event(event_id: int, user: CurrentUser = Depends(get_current_user)):
    return await orm_call(event_action, event_id=event_id, action="button_cancel", uid=_uid(user), context=_ctx(user))

@router.get("/events/{event_id}/registrations")
async def get_registrations(event_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def create_event_registration(body: RegistrationCreate, user: CurrentUser = Depends(get_current_user)):
...
async def get_surveys(params: SurveyListParams | None = None, user: CurrentUser = Depends(get_current_user)):
    p = params or SurveyListParams()
    return await orm_call(list_surveys, params=p.model_dump(), uid=_uid(user), context=_ctx(user))

@router.get("/surveys/{survey_id}")
async def get_survey_detail(survey_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def update_survey_detail(survey_id: int, body: SurveyUpdate, user: CurrentUser = Depends(get_current_user)):
...
async def create_survey(body: SurveyCreate, user: CurrentUser = Depends(get_current_user)):
...
async def start_survey(survey_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def close_survey(survey_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_survey_answers(survey_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_mailings(params: MailingListParams | None = None, user: CurrentUser = Depends(get_current_user)):
...
async def get_mailing_detail(mailing_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def update_mailing_detail(mailing_id: int, body: MailingUpdate, user: CurrentUser = Depends(get_current_user)):
...
async def create_mailing(body: MailingCreate, user: CurrentUser = Depends(get_current_user)):
...
async def schedule_mailing(mailing_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def cancel_mailing(mailing_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def send_mailing(mailing_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def test_mailing(mailing_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_mailing_statistics(mailing_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_pos_sessions(params: PosSessionListParams | None = None, user: CurrentUser = Depends(get_current_user)):
...
async def get_pos_session_detail(session_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_session_orders(session_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def open_pos_session(session_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def close_pos_session(session_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_pos_orders(params: PosOrderListParams | None = None, user: CurrentUser = Depends(get_current_user)):
...
async def get_pos_order_detail(order_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def get_calendar_events(params: CalendarEventListParams | None = None, user: CurrentUser = Depends(get_current_user)):
...
async def get_calendar_event_detail(event_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def update_calendar_event_detail(event_id: int, body: CalendarEventUpdate, user: CurrentUser = Depends(get_current_user)):
    vals = body.model_dump(exclude_none=True)
...
async def delete_calendar_event_endpoint(event_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def create_calendar_event(body: CalendarEventCreate, user: CurrentUser = Depends(get_current_user)):
    vals = body.model_dump(exclude_none=True)
    if "partner_ids" in vals:
        vals["partner_ids"] = [(6, 0, vals["partner_ids"])]
...
async def accept_calendar_event(event_id: int, user: CurrentUser = Depends(get_current_user)):
...
async def decline_calendar_event(event_id: int, user: CurrentUser = Depends(get_current_user)):
