...
from mashora import api, fields, models, _, SUPERUSER_ID, modules
from mashora.tools.sql import column_exists, create_column
from mashora.addons.account.tools import format_structured_reference_iso
from mashora.exceptions import UserError, ValidationError, AccessError, RedirectWarning
from mashora.fields import Command, Domain
...
from mashora.tools.mail import email_re, email_split, is_html_empty, generate_tracking_message_id
from mashora.tools.misc import StackMap
from mashora.tools.safe_eval import safe_eval
...
class AccountMove(models.Model):
    ...
    quick_edit_total_amount = fields.Monetary(
        ...
        help='Use this field to encode the total amount of the invoice.\n'
             'Mashora will automatically create one invoice line with default values to match it.',
    )
    ...
    def _get_invoice_reference_number_invoice(self):
        ...
        """
        ref = self._get_invoice_reference_mashora_invoice() or ''
    ...
    def _get_invoice_reference_number_partner(self):
        ...
        """
        ref = self._get_invoice_reference_mashora_partner()
        return ''.join(char for char in ref if char.isdigit())

    def _get_invoice_reference_mashora_invoice(self):
        """ This computes the reference based on the Mashora format.
    ...
    def _get_invoice_reference_mashora_partner(self):
        """ This computes the reference based on the Mashora format.
            The data used is the reference set on the partner or its database
            id otherwise. For instance if the reference of the customer is
    ...
    def _get_fields_to_copy_recurring_entries(self, values):
        ...
        values.update({
            ...
            'invoice_user_id': self.invoice_user_id.id,  # otherwise user would be MashoraBot
        })
    ...
    def _message_post_after_hook(self, new_message, message_values):
        ...
        if self.env.context.get('from_alias'):
            ...
            # Only trigger decoding if the message was sent by an active internal user (note MashoraBot is always inactive).
