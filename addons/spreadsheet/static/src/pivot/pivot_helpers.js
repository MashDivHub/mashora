...
import { EvaluationError, helpers } from "@mashora/o-spreadsheet";

const { isDateOrDatetimeField } = helpers;

/**
 * @typedef {import("@mashora/o-spreadsheet").Token} Token
 * @typedef {import("@mashora/o-spreadsheet").Granularity} Granularity
 * */
...
export const MASHORA_AGGREGATORS = {};

for (const type in AGGREGATORS_BY_FIELD_TYPE) {
    MASHORA_AGGREGATORS[type] = {};
    for (const aggregator of AGGREGATORS_BY_FIELD_TYPE[type]) {
        MASHORA_AGGREGATORS[type][aggregator] = AGGREGATOR_NAMES[aggregator];
    }
}
 ...
 * @param {Record<string, MashoraField | undefined>} allFields
 * @param {string} groupFieldString
 * @returns {{field: MashoraField, granularity: Granularity, isPositional: boolean, dimensionWithGranularity: string}}
 */
