...
export class MashoraChartFeaturePlugin extends MashoraUIPlugin {
    ...
    getAvailableChartGranularities(chartId) {
        ...
        const matching = this.getters.getMashoraChartFieldMatching(chartId, filterId);
    ...
    _getChartHorizontalAxisFilter(chartId, fieldName) {
        for (const filter of this.getters.getGlobalFilters()) {
            const matching = this.getters.getMashoraChartFieldMatching(chartId, filter.id);
