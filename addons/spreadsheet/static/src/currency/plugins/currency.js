import { EvaluationError, helpers, registries } from "@mashora/o-spreadsheet";
...
export class CurrencyPlugin extends MashoraUIPlugin {
    ...
    constructor(config) {
        ...
        this._serverData = config.custom.mashoraDataProvider?.serverData;
    }

    get serverData() {
        if (!this._serverData) {
            throw new Error(
                "'serverData' is not defined, please make sure a 'MashoraDataProvider' instance is provided to the model."
            );
        }
...
featurePluginRegistry.add("mashoraCurrency", CurrencyPlugin);
