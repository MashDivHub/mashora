import { animationFrame, describe, expect, test } from "@mashora/hoot";
import { Model, registries } from "@mashora/o-spreadsheet";
...
import { freezeMashoraData, waitForDataLoaded } from "@spreadsheet/helpers/model";
import { MashoraPivot, MashoraPivotRuntimeDefinition } from "@spreadsheet/pivot/mashora_pivot";
...
test("mashora pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeMashoraData(model);
...
test("Pivot with a type different of MASHORA is not converted", async function () {
    // Add a pivot with a type different of MASHORA
...
test("mashora pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeMashoraData(model);
...
test("mashora charts are replaced with an image", async function () {
...
test("Carousels figure with mashora data is converted to an image", async function () {
    const { model } = await createSpreadsheetWithChart({ type: "mashora_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Empty MASHORA.LIST result is frozen to an empty string", async function () {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", '=MASHORA.LIST(1, 9999,"probability")'); // has no record
    await waitForDataLoaded(model);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    const frozenData = await freezeMashoraData(model);
    expect(frozenData.sheets[0].cells.A1).toBe('=""');
});

test("mashora links are replaced with their label", async function () {
    const view = {
        name: "an mashora view",
        viewType: "list",
        action: {
            modelName: "partner",
            views: [[false, "list"]],
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: "[menu_xml](mashora://ir_menu_xml_id/test_menu)",
                    A2: "[menu_id](mashora://ir_menu_id/12)",
                    A3: `[mashora_view](mashora://view/${JSON.stringify(view)})`,
                    A4: "[external_link](https://mashora.com)",
    ...
    expect(frozenData.sheets[0].cells.A3).toBe("mashora_view");
    expect(frozenData.sheets[0].cells.A4).toBe("[external_link](https://mashora.com)");
