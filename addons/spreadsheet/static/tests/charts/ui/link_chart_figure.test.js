import { click } from "@mashora/hoot-dom";
import { animationFrame } from "@mashora/hoot-mock";
import { expect, test, beforeEach } from "@mashora/hoot";
...
test("icon external link isn't on the chart when its not linked to an mashora menu", async function () {
    ...
    await animationFrame();
    const mashoraMenu = model.getters.getChartMashoraMenu(chartId);
    expect(mashoraMenu).toBe(undefined, { message: "No menu linked with the chart" });

    const externalRefIcon = fixture.querySelector(".o-chart-external-link");
    expect(externalRefIcon).toBe(null);
});

test("icon external link is on the chart when its linked to an mashora menu", async function () {
    const { model } = await createModelWithDataSource({
        serverData,
    });
    await mountSpreadsheet(model);
    createBasicChart(model, chartId);
    model.dispatch("LINK_MASHORA_MENU_TO_CHART", {
        chartId,
        mashoraMenuId: 1,
    });

    const chartMenu = model.getters.getChartMashoraMenu(chartId);
    expect(chartMenu.id).toBe(1, { message: "Mashora menu is linked to chart" });
    await animationFrame();
    expect(".o-chart-external-link").toHaveCount(1);
});

test("icon external link is not on the chart when its linked to a wrong mashora menu", async function () {
    const { model } = await createModelWithDataSource({
        serverData,
    });
    await mountSpreadsheet(model);
    ...
    model.dispatch("LINK_MASHORA_MENU_TO_CHART", {
        chartId,
        mashoraMenuId: "menu which does not exist",
    });
    const chartMenu = model.getters.getChartMashoraMenu(chartId);
...
test("click on icon external link on chart redirect to the mashora menu", async function () {
    ...
    const chartMenu = model.getters.getChartMashoraMenu(chartId);
    expect(chartMenu.id).toBe(2, { message: "Mashora menu is linked to chart" });
    await animationFrame();
...
test("can use menus xmlIds instead of menu ids", async function () {
    ...
    model.dispatch("LINK_MASHORA_MENU_TO_CHART", {
        chartId,
        mashoraMenuId: "spreadsheet.test.menu2",
    });
    await animationFrame();
...
test("Trying to open a menu without an action sends a notification to the user", async function () {
    ...
    model.dispatch("LINK_MASHORA_MENU_TO_CHART", {
        chartId,
        mashoraMenuId: "spreadsheet.test.menu_without_action",
    });
    await animationFrame();
