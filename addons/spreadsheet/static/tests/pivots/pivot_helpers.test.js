...
import { constants, tokenize, helpers } from "@mashora/o-spreadsheet";
...
test("Basic formula extractor", async function () {
    const formula = `=PIVOT.VALUE("1", "test") + MASHORA.LIST("2", "hello", "bla")`;
    ...
    expect(functionName).toBe("MASHORA.LIST");
...
test("Number of formulas", async function () {
    const formula = `=PIVOT.VALUE("1", "test") + PIVOT.VALUE("2", "hello", "bla") + MASHORA.LIST("1", "bla")`;
