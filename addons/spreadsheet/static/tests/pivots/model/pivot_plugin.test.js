import { describe, expect, test, Deferred } from "@mashora/hoot";
        ...
        sheets: [
            {
                id: "sheet1",
                cells: {
                    A1: '=MASHORA.PIVOT(1, "probability")',
                },
            },
        ],
        pivots: {
...
test("evaluates only once when two pivots are loading", async function () {
    ...
    model.config.custom.mashoraDataProvider.addEventListener("data-source-updated", () =>
...
test("can edit pivot domain with UPDATE_MASHORA_PIVOT_DOMAIN", async () => {
    const { model } = await createSpreadsheetWithPivot();
    const [pivotId] = model.getters.getPivotIds();
