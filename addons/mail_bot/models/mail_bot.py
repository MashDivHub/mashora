...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, channel, values, command=None):
        ...
        The logic will only be applied if mashorabot is in a chat with a user or
        if someone pinged mashorabot.

         :param channel: the discuss channel where the user message was posted/mashorabot will answer.
        ...
        if values.get("author_id") == mashorabot_id or values.get("message_type") != "comment" and not command:
            return
    ...
    def _get_style_dict():
        return {
            ...
            "bold_start": Markup("<b>"),
            "bold_end": Markup("</b>"),
            "command_start": Markup("<span class='o_mashorabot_command'>"),
            "command_end": Markup("</span>"),
            "document_link_start": Markup("<a href='https://www.mashora.com/documentation' target='_blank'>"),
            "document_link_end": Markup("</a>"),
            "slides_link_start": Markup("<a href='https://www.mashora.com/slides' target='_blank'>"),
    ...
    def _get_answer(self, channel, body, values, command=False):
        ...
        # onboarding
        mashorabot_state = self.env.user.mashorabot_state

        if channel.channel_type == "chat" and mashorabot in channel.channel_member_ids.partner_id:
            # main flow
            source = _("Thanks")
            description = _("This is a temporary canned response to see how canned responses work.")
            if mashorabot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.mashorabot_state = "onboarding_command"
                self.env.user.mashorabot_failed = False
                return self.env._(
            ...
            elif mashorabot_state == 'onboarding_command' and command == 'help':
                self.env.user.mashorabot_state = "onboarding_ping"
                self.env.user.mashorabot_failed = False
                return self.env._(
                    ...
                    "%(command_start)s@MashoraBot%(command_end)s in a sentence.",
                    **self._get_style_dict()
                )
            elif mashorabot_state == "onboarding_ping" and mashorabot.id in values.get("partner_ids", []):
                self.env.user.mashorabot_state = "onboarding_attachement"
                self.env.user.mashorabot_failed = False
                return self.env._(
                ...
                return [
                    ...
                    self.env._(
                        ...
                        "%(command_start)sstart the tour%(command_end)s to see it again. Enjoy exploring Mashora!",
                        **self._get_style_dict(),
                    ),
                ]
            # repeat question if needed
            elif mashorabot_state == 'onboarding_canned' and not self._is_help_requested(body):
                self.env.user.mashorabot_failed = True
                return self.env._(
            ...
            elif mashorabot_state in (False, "idle", "not_initialized") and (_('start the tour') in body.lower()):
                self.env.user.mashorabot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif mashorabot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or mashorabot_state == 'idle':
                return self.env._(
                ...
                # repeat question
                if mashorabot_state == 'onboarding_emoji':
                    self.env.user.mashorabot_failed = True
                    return self.env._(
                ...
                elif mashorabot_state == 'onboarding_attachement':
                    self.env.user.mashorabot_failed = True
                    return self.env._(
                ...
                elif mashorabot_state == 'onboarding_command':
                    self.env.user.mashorabot_failed = True
                    return self.env._(
                ...
                elif mashorabot_state == 'onboarding_ping':
                    self.env.user.mashorabot_failed = True
                    return self.env._(
                        "Sorry, I am not listening. To get someone's attention, %(bold_start)sping "
                        "him%(bold_end)s. Write %(command_start)s@MashoraBot%(command_end)s and select"
                        " me.",
                        **self._get_style_dict()
                    )
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.mashorabot_failed
