...
import { advanceTime, animationFrame, tick, waitUntil } from "@mashora/hoot-dom";
import { Component, xml, markup } from "@mashora/owl";
...
class PeerTest {
    ...
    async focus() {
        return this.plugins["collaborationMashora"].joinPeerToPeer();
    }
    ...
    async writeToServer() {
        ...
        const lastId = this.plugins.collaborationMashora.getLastHistoryStepId(
        ...
        for (const peer of Object.values(this.peers)) {
            ...
            peer.onlineMutex.exec(async () =>
                peer.plugins.collaborationMashora.onServerLastIdUpdate(String(lastId))
            );
        }
    }
    async setOnline() {
...
class Wysiwygs extends Component {
    ...
    onLoad(peerId, editor) {
        ...
        editor.startPlugins = () => {
            ...
            patch(plugins["collaborationMashora"], {
                getMetadata() {
...
describe("Stale detection & recovery", () => {
    describe("detect stale while unfocused", () => {
        test("should do nothing until focus", async () => {
            ...
            expect(peers.p1.plugins.collaborationMashora.isDocumentStale).toBe(false, {
            ...
            expect(peers.p2.plugins.collaborationMashora.isDocumentStale).toBe(false, {
            ...
            expect(peers.p3.plugins.collaborationMashora.isDocumentStale).toBe(true, {
            ...
            expect(peers.p3.plugins.collaborationMashora.isDocumentStale).toBe(false, {
    ...
    describe("detect stale while focused", () => {
        describe("recover from missing steps", () => {
            test("should recover from missing steps", async () => {
                ...
                const p3Spies = makeSpies(peers.p3.plugins.collaborationMashora, [
                ...
                expect(peers.p1.plugins.collaborationMashora.historyShareId).toBe(
                    peers.p2.plugins.collaborationMashora.historyShareId,
                    {
                        message: "p1 and p2 should have the same historyShareId",
                    }
                );
                expect(peers.p1.plugins.collaborationMashora.historyShareId).toBe(
                    peers.p3.plugins.collaborationMashora.historyShareId,
                    {
        ...
        describe("recover from snapshot", () => {
            test("should wait for all peer to recover from snapshot", async () => {
                ...
                const p2Spies = makeSpies(peers.p2.plugins.collaborationMashora, [
                    "recoverFromStaleDocument",
                    "resetFromServerAndResyncWithPeers",
                    "processMissingSteps",
                    "applySnapshot",
...
describe("Disconnect & reconnect", () => {
    test("should sync history when disconnecting and reconnecting to internet", async () => {
        ...
        const p1PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p2.plugins.collaborationMashora, {
        ...
        const p2PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p1.plugins.collaborationMashora, {
                async processMissingSteps() {
                    super.processMissingSteps(...arguments);
                    resolve();
                },
...
describe("History steps Ids", () => {
    ...
    test("should clear history step ids when resetting from server", async () => {
        ...
        expect(peers.p2.plugins.collaborationMashora.isDocumentStale).toBe(true, {
