...
import { beforeEach, describe, expect, getFixture, test } from "@mashora/hoot";
import { click, queryFirst } from "@mashora/hoot-dom";
    ...
    test("Can mount and destroy recursive embedded components in any order", async () => {
        ...
        expect(
            [...fixture.querySelectorAll("[data-embedded]")].filter(
                (elem) => !elem.closest(".mashora-editor-editable")
            )
        ).toEqual([]);
    });
