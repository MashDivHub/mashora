...
from mashora import http, tests
from mashora.addons.base.tests.common import HttpCaseWithUserPortal
from mashora.addons.gamification.tests.common import HttpCaseGamification
from mashora.fields import Command, Datetime
...
class TestUi(TestUICommon):
    ...
    @mute_logger("mashora.http", "mashora.addons.base.models.ir_rule", "werkzeug")
    ...
    def test_review_link_redirection(self):
        ...
        cases = [
            ("admin", rf"^/mashora/slide.channel/.*highlight_message_id={message.id}"),
...
class TestUiPublisher(HttpCaseGamification):
    ...
    def fetch_proxy(self, url):
        if url.endswith('ThreeTimeAKCGoldWinnerPembrookeWelshCorgi.jpg'):
            ...
            with file_open('base/tests/mashora.jpg', 'rb') as f:
                content = f.read()
