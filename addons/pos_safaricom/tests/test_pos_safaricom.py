...
class TestPosSafaricom(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        with patch('mashora.addons.pos_safaricom.models.pos_payment_method.PosPaymentMethod.lipa_na_mpesa_register_urls'):
    ...
    def test_bearer_token_success(self, mock_get):
        ...
        token = self.mpesa_express._get_bearer_token()
    ...
    def test_bearer_token_missing_credentials(self, mock_get):
        ...
        with self.assertRaises(UserError) as context:
            payment_method._get_bearer_token()

        self.assertIn('Consumer Key and Consumer Secret are required', str(context.exception))

    @patch('requests.post')
    @patch('mashora.addons.pos_safaricom.models.pos_payment_method.PosPaymentMethod._get_bearer_token')
