...
test("MASHORA.ACCOUNT.GROUP type", async function () {
    const { store: composer } = await makeStore(CellComposerStore);

    composer.startEdition("=MASHORA.ACCOUNT.GROUP(");
    ...
    await animationFrame();
    expect(composer.currentContent).toBe('=MASHORA.ACCOUNT.GROUP("asset_receivable"');
