...
featurePluginRegistry.add("mashoraAccountingAggregates", AccountingPlugin);
...
cellMenuRegistry.add("move_lines_see_records", {
    ...
    async execute(env, newWindow) {
        ...
        if ( func.functionName === "MASHORA.PARTNER.BALANCE" ) {
            [partner_ids, codes, date_range, offset, companyId, includeUnposted] = parsed_args;
        } else if ( func.functionName === "MASHORA.BALANCE.TAG" ) {
        ...
        if ( date_range?.value && !isEvaluationError(date_range.value) ) {
            ...
            if ( ["MASHORA.PARTNER.BALANCE", "MASHORA.RESIDUAL", "MASHORA.BALANCE.TAG"].includes(func.functionName) ) {
        ...
        if ( func.functionName === "MASHORA.BALANCE.TAG" ) {
        ...
        } else if ( func.functionName === "MASHORA.PARTNER.BALANCE" ) {
