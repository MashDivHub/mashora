...
from mashora import http, tools
...
class DriverController(http.Controller):
        ...
        if device_identifier == helpers.get_identifier():
            match data.get('action'):
                case "restart_mashora":
                    ...
                    return helpers.mashora_restart()
                case _:
    ...
    def download_logs(self):
        ...
        """
        log_path = tools.config['logfile'] or "/var/log/mashora/mashora-server.log"
        try:
        ...
        log_file_name = f"iot-mashora-{gethostname()}-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
        # intentionally don't use Stream.from_path as the path used is not in the addons path
        # for instance, for the iot-box it will be in /var/log/mashora
        return http.Stream(
                type='path',
                path=log_path,
