...
from mashora.addons.iot_drivers.tools import helpers
from mashora.addons.iot_drivers.tools.system import IS_TEST
from mashora.netsvc import ColoredFormatter

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in mashora.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, mashora_server_url, active):
        """
        :param mashora_server_url: Mashora Server URL
        """
        super().__init__()
        self._mashora_server_url = mashora_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._mashora_server_url:
    ...
    def _periodic_flush(self):
        mashora_session = requests.Session()
        while self._mashora_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(mashora_session)

    def _flush_logs(self, mashora_session):
        ...
        if not self._mashora_server_url or queue_size == 0:
            return
        try:
            mashora_session.post(
                self._mashora_server_url + '/iot/log',
                data=empty_queue(),
                headers={'X-Mashora-Database': self._db_name},
...
def get_mashora_config_log_to_server_option():
...
def check_and_update_mashora_config_log_to_server_option(new_state):
    """
    :return: wherever the config file need to be updated or not
    """
    if get_mashora_config_log_to_server_option() != new_state:
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_mashora_server_url(), get_mashora_config_log_to_server_option())
if not IS_TEST:
    ...
    # Set it in the 'root' logger, on which every logger (including mashora) is a child
