...
from mashora import http, models, fields, tools, _
from mashora.exceptions import AccessError, UserError
from mashora.fields import Domain
from mashora.http import request, SessionExpiredException
from mashora.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from mashora.tools.translate import LazyTranslate
from mashora.addons.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
from mashora.addons.portal.controllers.portal import pager as portal_pager
from mashora.addons.portal.controllers.web import Home
from mashora.addons.web.controllers.binary import Binary
from mashora.addons.web.controllers.session import Session
from mashora.addons.website.tools import get_base_domain
from mashora.tools.json import scriptsafe as json
from mashora.tools.translate import TRANSLATED_ELEMENTS
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/mashora?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_info(self, **kwargs):
        ...
        values = {
            'apps': apps,
            'l10n': l10n,
            'version': mashora.service.common.exp_version()
        }
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/mashora/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/mashora/ir.ui.view/{page.get('view_id')}")
