...
from mashora.addons.point_of_sale.tests.common import TestPoSCommon
from mashora.exceptions import ValidationError
from freezegun import freeze_time
from dateutil.relativedelta import relativedelta
from datetime import datetime, timedelta
import unittest.mock
from mashora.http import UserError
...
class TestPoSBasicConfig(TestPoSCommon):
    ...
    def test_cash_register_if_no_order(self):
        ...
        with (
            self.assertLogs('mashora.addons.point_of_sale.models.pos_order', level='DEBUG') as cm,
            unittest.mock.patch('mashora.addons.point_of_sale.models.pos_order.randrange', return_value=1996)
        ):
            ...
            mashora_order_id = res['pos.order'][0]['id']
            self.assertEqual(len(cm.output), 4)
            self.assertEqual(cm.output[0], f"INFO:mashora.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 started for PoS orders references: [{order_log_str}]")
            self.assertTrue(cm.output[1].startswith(f'DEBUG:mashora.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 processing order {order_log_str} order full data: '))
            self.assertEqual(cm.output[2], f'INFO:mashora.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 order {order_log_str} created pos.order #{mashora_order_id}')
            self.assertEqual(cm.output[3], 'INFO:mashora.addons.point_of_sale.models.pos_order:PoS synchronisation #1996 finished')
