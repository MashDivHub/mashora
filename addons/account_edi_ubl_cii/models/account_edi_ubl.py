...
from mashora.tools import frozendict, html2plaintext, pdf
...
class AccountEdiUBL(models.AbstractModel):
    ...
    def _import_attachments(self, invoice, tree):
        ...
        try:
            invoices_by_mashora_xmlid = 'account_edi_ubl_cii.action_report_account_invoices_generated_by_mashora'
            report_xmlid = invoices_by_mashora_xmlid if self.env.ref(invoices_by_mashora_xmlid, raise_if_not_found=False) else 'account.account_invoices'
            ...
            with io.BytesIO(pdf_raw) as pdf_stream:
                new_pdf_stream = pdf.add_banner(pdf_stream, _('Generated by Mashora'), logo=False)
                pdf_raw = new_pdf_stream.getvalue()
                new_pdf_stream.close()

            invoice_name = invoice.display_name.replace(_('Draft'), '')
            pdf_filename = _('%(invoice_name)s - Generated by Mashora', invoice_name=invoice_name)
