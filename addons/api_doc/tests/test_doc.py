...
from mashora.tests import new_test_user, tagged

from .dummy_methods import DummyMethods
from mashora.addons.api_doc.controllers.api_doc import (
    DocController,
    is_public_method,
    parse_signature,
)
...
class TestDoc(HttpCaseWithUserDemo):
    ...
    def test_doc_access(self):
        ...
        for path in ('/doc', '/doc/index.json', '/doc/res.company.json'):
            with self.subTest(path=path):
                ...
                self.assertEqual(capture.output, [f'WARNING:mashora.http:{e}'])
    ...
    def test_doc_cache(self):
        ...
        self.assertRegex(
            ...
            r'mashora-doc-index-\w+-%s\.json' % etag_demo.strip('"'),
        ...
        self.assertRegex(res.headers.get('Content-Disposition', ''), r'\bmashora-doc-index\.json$',
    ...
    def test_private_methods(self):
        FakeCls = type('ModelDummyMethods', (DummyMethods, Model), {
            ...
            '_register': False,
            '__module__': 'mashora.addons.api_doc',
        })
