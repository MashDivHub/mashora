...
/**
 * @typedef {import("../../env").MashoraEnv} MashoraEnv
...
export function swallowAllVisitorErrors(env, error, originalError) {
    if (!user.isInternalUser && !mashora.debug && !session.test_mode) {
        return true;
    }
}
...
if (user.isInternalUser === undefined) {
    ...
    // apparently not be present in all Mashora screens at the moment... TODO ?
