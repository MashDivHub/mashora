...
export function startUrl() {
    return isScopedApp() ? "scoped_app" : "mashora";
}
...
function urlToState(urlObj) {
    ...
    if (["mashora", "scoped_app"].includes(prefix)) {
        ...
        if (prefix === "scoped_app" && !isDisplayStandalone()) {
            // make sure /scoped_app are redirected to /mashora when using the browser instead of the PWA
            const url = browser.location.origin + router.stateToUrl(state);
            urlObj.href = url;
        }
    }
 ...
 * external website). Unfortunately, Mashora wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/mashora") &&
            (["/web", "/mashora"].includes(url.pathname) || url.pathname.startsWith("/mashora/")) &&
            a.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/mashora") && url.hash) {
