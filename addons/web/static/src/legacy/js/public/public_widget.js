...
function MashoraEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
MashoraEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
MashoraEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Mashora where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new MashoraEvent(this, name, info);
