...
try:
    ...
    from mashora.tools._vendor.send_file import send_file

import mashora
import mashora.modules.registry
from mashora import SUPERUSER_ID, _, http, api
from mashora.addons.base.models.assetsbundle import ANY_UNIQUE
from mashora.exceptions import AccessError, UserError
from mashora.http import request, Response
from mashora.tools import file_open, file_path, replace_exceptions, str2bool
from mashora.tools.image import image_guess_size_from_field_name
from mashora.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Mashora is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Mashora is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if mashora.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=mashora.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or mashora.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the Mashora logo instead", exc_info=True)
