...
class TestWebRedirect(HttpCase):
    ...
    def test_web_route_redirect_param(self):
        # This test if for the new routes with /mashora, pathname and query params
        web_response = self.url_open('/mashora/action-887?cids=1')
        web_response.raise_for_status()
        response_url_query = url_parse(web_response.url).query

        self.assertEqual(response_url_query, 'redirect=%2Fmashora%2Faction-887%3Fcids%3D1')
