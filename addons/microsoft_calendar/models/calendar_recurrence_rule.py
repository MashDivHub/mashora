...
class CalendarRecurrence(models.Model):
    ...
    def _inverse_rrule(self):
        ...
        # modified in Mashora but computed from other fields).
    ...
    def _write_from_microsoft(self, microsoft_event, vals):
        ...
        # event_tz is written on event in Microsoft but on recurrence in Mashora
        vals['event_tz'] = microsoft_event.start.get('timeZone')
        super()._write_from_microsoft(microsoft_event, vals)
        new_event_values = self.env["calendar.event"]._microsoft_to_mashora_values(microsoft_event)
    ...
    def _get_microsoft_sync_domain(self):
        # Do not sync Mashora recurrences with Outlook Calendar anymore.
    ...
    @api.model
    def _microsoft_to_mashora_values(self, microsoft_recurrence, default_reminders=(), default_values=None, with_ids=False):
