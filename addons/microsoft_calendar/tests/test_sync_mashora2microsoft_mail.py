...
from mashora.addons.microsoft_calendar.tests.common import TestCommon


class TestSyncMashora2MicrosoftMail(TestCommon, MailCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
    ...
    def test_change_organizer_pure_mashora_event(self):
        """
        Test that changing organizer on a pure Mashora event (not synced with Microsoft)
        ...
        self.assertTrue(event.active, "Pure Mashora event should not be archived when changing organizer")
