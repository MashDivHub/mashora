...
from mashora.exceptions import AccessDenied, AccessError, UserError
from mashora.fields import Domain
from mashora.http import request
from mashora.modules.module import MANIFEST_NAMES, Manifest
from mashora.release import major_version
from mashora.tools import SQL, convert_file
from mashora.tools import file_open, file_path, file_open_temporary_directory, ormcache
from mashora.tools.misc import OrderedSet, topological_sort
from mashora.tools.translate import JAVASCRIPT_TRANSLATION_COMMENT, CodeTranslations, TranslationImporter, get_base_langs

from mashora.addons.base.models.ir_asset import is_wildcard_glob

_logger = logging.getLogger(__name__)

APPS_URL = "https://apps.mashora.com"
...
class IrModuleModule(models.Model):
    ...
    def _import_module(self, module, path, force=False, with_demo=False):
        ...
        if unmet_dependencies:
            ...
            raise UserError(_("Studio customizations require the Mashora Studio app."))
    ...
    def _get_missing_dependencies(self, zip_data):
        ...
        if unavailable_modules:
            ...
            description += _(
                ...
                "https://www.mashora.com/pricing-plan for more information.\n"
                "If you need Website themes, it can be downloaded from https://github.com/mashora/design-themes.\n"
            )
        else:
            description = _(
    ...
    def _extract_resource_attachment_translations(self, module, lang):
        ...
        for attachment in attachments.filtered('raw'):
            ...
            if attachment.url.endswith('js'):
                extract_method = 'mashora.tools.babel:extract_javascript'
                extract_keywords = {'_t': None}
            else:
                extract_method = 'mashora.tools.translate:babel_extract_qweb'
                extract_keywords = {}
            try:
                with io.BytesIO(attachment.raw) as fileobj:
