...
from .account_edi_proxy_auth import MashoraEdiProxyAuth
    ...
    def _make_request(self, url, params=False, *, auth_type: Literal['hmac', 'asymmetric'] = 'hmac'):
        ...
        try:
            res = requests.post(
                ...
                auth=MashoraEdiProxyAuth(user=self, auth_type=auth_type))
        ...
        if proxy_error:
            ...
            if error_code == 'invalid_signature':
                raise AccountEdiProxyError(
                    error_code,
                    _("Failed to connect to Mashora Access Point server. This might be due to another connection to Mashora Access Point "
        ...
        if edi_mode == 'demo':
            ...
            if 'error' in response:
                if response['error'] == 'A user already exists with this identification.':
                    ...
                    # Mashora side. We are for now forced to check the error message.
