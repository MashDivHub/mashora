...
from mashora.http import Response
from mashora.tests import Form, tagged
from mashora.tools.misc import file_open

from mashora.addons.sale_management.tests.common import SaleManagementCommon
from mashora.addons.sale_pdf_quote_builder.controllers.quotation_document import (
...
class TestPDFQuoteBuilder(SaleManagementCommon):
    ...
    def setUpClass(cls):
        ...
        cls.internal_user = cls._create_new_internal_user(login='internal.user@test.mashora.com', groups='sales_team.group_sale_salesman')
    ...
    def test_quotation_document_upload_no_template(self):
        ...
        if 'website' not in self.env:
            ...
            from mashora.addons.http_routing.tests.common import MockRequest  # noqa: PLC0415
    ...
    def _test_custom_content_kanban_like(self):
        ...
        self.start_tour(
            f'/mashora/sales/{self.sale_order.id}',
