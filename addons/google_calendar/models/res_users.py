...
from mashora import api, fields, models, Command
from mashora.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from mashora.addons.google_calendar.models.google_sync import google_calendar_token
from mashora.addons.google_account.models import google_service
from mashora.exceptions import LockError
from mashora.loglevels import exception_to_unicode
from mashora.tools import str2bool
...
class ResUsers(models.Model):
    ...
    def _check_pending_mashora_records(self):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        if not events and not self._check_pending_mashora_records():
            return False
        # Google -> Mashora
        ...
        # We apply Google updates only if their write date is later than the write date in Mashora.
        # It's possible that multiple updates affect the same record, maybe not directly.
        # To handle this, we preserve the write dates in Mashora before applying any updates,
        # and use these dates instead of the current live dates.
        mashora_events = self.env['calendar.event'].browse((events - recurrences).mashora_ids(self.env))
        mashora_recurrences = self.env['calendar.recurrence'].browse(recurrences.mashora_ids(self.env))
        recurrences_write_dates = {r.id: r.write_date for r in mashora_recurrences}
        events_write_dates = {e.id: e.write_date for e in mashora_events}
        synced_recurrences = self.env['calendar.recurrence']._sync_google2mashora(recurrences, recurrences_write_dates)
        synced_events = self.env['calendar.event']._sync_google2mashora(events - recurrences, events_write_dates, default_reminders=default_reminders)

        # Mashora -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_mashora2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_mashora2google(calendar_service)
    ...
    def _sync_google_calendar_filter_remote_events(self, google_events):
        """Filter out events coming from google which should not be synced into mashora."""
    ...
    def _sync_single_event(self, calendar_service: GoogleCalendarService, mashora_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2mashora(event, default_reminders=default_reminders)
        # Mashora -> Google
        mashora_event.with_context(send_updates=send_updates)._sync_mashora2google(calendar_service)
        return bool(mashora_event | synced_events)
