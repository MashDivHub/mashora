...
from mashora.addons.payment_toss_payments import const
from mashora.addons.payment_toss_payments.controllers.main import TossPaymentsController
from mashora.addons.payment_toss_payments.tests.common import TossPaymentsCommon


@tagged('post_install', '-at_install')
class TestProcessingFlows(TossPaymentsCommon, PaymentHttpCommon):
    @mute_logger('mashora.addons.payment_toss_payments.controllers.main')
    def test_returning_from_successful_payment_initiation_triggers_processing(self):
        ...
        with (
            patch(
                'mashora.addons.payment.models.payment_transaction.PaymentTransaction'
                '._send_api_request'
            ),
            patch(
    ...
    def test_failing_to_confirm_payment_sets_the_transaction_in_error(self):
        ...
        url = self._build_url(const.PAYMENT_SUCCESS_RETURN_ROUTE)
        with patch(
            'mashora.addons.payment.models.payment_transaction.PaymentTransaction._send_api_request',
    ...
    def test_webhook_notification_triggers_processing(self):
        ...
        with (
            patch(
                'mashora.addons.payment_toss_payments.controllers.main.TossPaymentsController'
                '._verify_signature'
            ),
            patch(
                'mashora.addons.payment.models.payment_transaction.PaymentTransaction._process'
    ...
    def test_webhook_notification_triggers_signature_check(self):
        ...
        with (
            ...
            patch('mashora.addons.payment.models.payment_transaction.PaymentTransaction._process'),
        ):
            self._make_json_request(url, data=self.webhook_data)
