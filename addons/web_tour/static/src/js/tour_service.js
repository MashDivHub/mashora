import { Component, markup, whenReady, validate } from "@mashora/owl";
    ...
    start: async (env, { orm, effect, overlay }) => {
        ...
        async function resumeTour() {
            ...
            if (tourConfig.mode === "auto") {
                if (!mashora.loader.modules.get("@web_tour/js/tour_automatic/tour_automatic")) {
                    await loadBundle("web_tour.automatic", { css: false });
                }
                const { TourAutomatic } = mashora.loader.modules.get(
                    "@web_tour/js/tour_automatic/tour_automatic"
                );
                new TourAutomatic(tour).start();
            } else {
                await loadBundle("web_tour.interactive");
                const { TourPointer } = mashora.loader.modules.get(
                ...
                const { TourInteractive } = mashora.loader.modules.get(
        ...
        async function tourRecorder() {
            ...
            const { TourRecorder } = mashora.loader.modules.get(
        ...
        mashora.startTour = startTour;
        mashora.isTourReady = (tourName) => getTourFromRegistry(tourName).wait_for.then(() => true);
