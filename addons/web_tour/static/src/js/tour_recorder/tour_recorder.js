...
import { queryAll, queryFirst, queryOne } from "@mashora/hoot-dom";
import { Component, useState, useExternalListener } from "@mashora/owl";
...
const MASHORA_CLASS_REGEX = /^oe?(-|_)[\w-]+$/;
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasMashoraClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasMashoraClass;
        ...
        const mashoraClass = [...currentElem.classList].find((c) => c.match(MASHORA_CLASS_REGEX));
        if (mashoraClass) {
            currentPredicate = `.${mashoraClass}`;
            hasMashoraClass = true;
        }
