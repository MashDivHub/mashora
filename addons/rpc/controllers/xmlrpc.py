...
import mashora.exceptions
from mashora.fields import Command, Date, Datetime
from mashora.http import Controller, Response, dispatch_rpc, request, route
from mashora.tools import lazy
from mashora.tools.misc import frozendict
...
# User code must use the exceptions defined in ``mashora.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, mashora.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, mashora.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, mashora.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, mashora.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, mashora.exceptions.MissingError):
    ...
    # InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(mashora.tools.exception_to_unicode(e), formatted_info)

    return dumps(fault)


class MashoraMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, mashora is sending binary as base64 string.
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = MashoraMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}
