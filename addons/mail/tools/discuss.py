...
from mashora.addons.bus.websocket import wsrequest
...
def get_sfu_url(env) -> str | None:
    ...
    if not sfu_url:
        sfu_url = os.getenv("MASHORA_SFU_URL")
    if sfu_url:
...
def get_sfu_key(env) -> str | None:
    ...
    if not sfu_key:
        return os.getenv("MASHORA_SFU_KEY")
    return sfu_key
    ...
    def _add_values(self, values, model_name, index=None):
        ...
        for key, val in values.items():
            ...
            if isinstance(val, Store.Relation):
                ...
                target[key] = mashora.fields.Datetime.to_string(val)
            elif isinstance(val, date):
                target[key] = mashora.fields.Date.to_string(val)
