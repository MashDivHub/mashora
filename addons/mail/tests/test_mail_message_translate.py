...
class TestTranslationController(HttpCaseWithUserDemo):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="mashora.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="mashora.exceptions.AccessError"), mute_logger("mashora.http"):
            self._mock_translation_request({"message_id": self.message.id})
