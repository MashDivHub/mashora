...
from mashora.tests.common import tagged, new_test_user, TransactionCase
from mashora.tools import mute_logger

from mashora.addons.base.tests.common import HttpCase
from mashora.addons.crm.tests.common import TestCrmCommon
from mashora.addons.mail.controllers.thread import ThreadController
...
from mashora.addons.website_crm_partner_assign.controllers.main import (
    WebsiteAccount,
...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('mashora.addons.base_geolocalize.models.base_geocoder.BaseGeocoder.geo_find', wraps=geo_find)
...
class TestPartnerLeadPortal(TestCrmCommon, HttpCase):
    ...
    @patch('mashora.http.GeoIP')
    def test_03_crm_partner_assign_geolocalization(self, GeoIpMock):
        """
            This test checks situation when "{MashoraURL}/partners" is visited from foreign country without resellers.
        ...
        """
        # Patch GeoIp so it acts, as if Mashora client is located in Mexico
